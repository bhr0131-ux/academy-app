...
export default function App() {
  ...
  const [editingChild,setEditingChild] = useState(null); // null=신규, id=수정
  ...
  useEffect(()=>{
    (async()=>{
      const ch=await load("v6_children"), ac=await load("v6_ac"), ab=await load("v6_abs"),
            ...
            tmpl=await load("v6_tmpl"), cid=await load("v6_cid");
  ...
  // 공통 스타일
  const inp={ width:"100%",boxSizing:"border-box",background:C.faint,border:`1px solid ${C.faintB}`,borderRadius:10,padding:"12px 14px",color:C.text,fontSize:15,outline:"none",fontFamily:"inherit" };
  const lbl={ fontSize:13,color:C.sub,display:"block",marginBottom:7,fontWeight:700 };

  if(!loaded) return (
    <div style={{background:C.bg,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column"}}>
      <div style={{fontSize:48}}>🎒</div>
      <p style={{color:C.sub,fontSize:16,marginTop:12}}>불러오는 중...</p>
    </div>
  );
  ...
  return (
    <div style={{fontFamily:"'Noto Sans KR','Apple SD Gothic Neo',sans-serif",background:C.bg,minHeight:"100vh",maxWidth:430,margin:"0 auto",color:C.text,paddingBottom:90}}>
      ...
      {/* 토스트 */}
      {toast&&<div style={{position:"fixed",top:20,left:"50%",transform:"translateX(-50%)",background:th.main,color:"#fff",padding:"10px 24px",borderRadius:20,fontSize:14,fontWeight:700,zIndex:999,boxShadow:`0 4px 16px ${th.main}55`}}>{toast}</div>}
      ...
      <div style={{display:"flex",background:C.card,borderBottom:`1px solid ${C.border}`,position:"sticky",top:0,zIndex:10,boxShadow:"0 2px 8px rgba(0,0,0,0.05)"}}>
        {[["home","🏠 홈"],["calendar","🗓 달력"],["fee","💰 학원비"],["absence","🏥 결석"],["sms","💬 문자"]].map(([k,l])=>(
          <button key={k} onClick={()=>setTab(k)} style={{flex:1,padding:"13px 2px",border:"none",background:"transparent",fontSize:11,fontWeight:tab===k?800:400,color:tab===k?th.main:C.sub,borderBottom:tab===k?`2.5px solid ${th.main}`:"2.5px solid transparent",cursor:"pointer",whiteSpace:"nowrap",transition:"color 0.2s"}}>{l}</button>
        ))}
      </div>
            ...
            <div>
                  ...
                  <div key={ac.id} style={{background:C.card,borderRadius:18,marginBottom:14,border:`1.5px solid ${ac.color}30`,boxShadow:`0 4px 18px ${ac.color}12`,overflow:"hidden"}}>
                    <div style={{background:`${ac.color}10`,padding:"14px 16px",display:"flex",alignItems:"center",gap:12}}>
                      ...
                      <div style={{flex:1}}>
                        ...
                        <p style={{fontSize:13,color:C.sub,margin:"3px 0 0"}}>{ac.time} ~ {endT} &nbsp;·&nbsp; {ac.duration}분</p>
                        {ac.teacher&&<p style={{fontSize:12,color:C.sub,margin:"2px 0 0"}}>👩‍🏫 {ac.teacher}</p>}
                      </div>
                      <div style={{display:"flex",gap:8}}>
                    ...
                    <div style={{padding:"14px 16px"}}>
                      ...
                      <div style={{marginBottom:12}}>
                        <p style={{fontSize:12,fontWeight:700,color:C.sub,margin:"0 0 7px",letterSpacing:0.5}}>🎒 준비물</p>
                        <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                          {(ac.baseSupplies||[]).map((s,i)=><span key={`b${i}`} style={{fontSize:12,padding:"4px 11px",borderRadius:20,background:`${ac.color}18`,color:ac.color,fontWeight:600}}>{s}</span>)}
                          {sup.map((s,i)=><span key={`d${i}`} style={{fontSize:12,padding:"4px 11px",borderRadius:20,background:`${C.orange}15`,color:C.orange,fontWeight:600}}>+{s}</span>)}
                          {(ac.baseSupplies||[]).length===0&&sup.length===0&&<span style={{fontSize:12,color:"#CCC"}}>없음</span>}
                        </div>
                      </div>
                      {/* 숙제 */}
                      <div style={{marginBottom:12}}>
                        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:7}}>
                          <p style={{fontSize:12,fontWeight:700,color:C.sub,margin:0,letterSpacing:0.5}}>📝 숙제</p>
                          {hw.length>0&&<span style={{fontSize:11,fontWeight:700,color:allDone?C.green:C.orange}}>{allDone?"✓ 완료":`${doneCnt}/${hw.length} 완료`}</span>}
                        </div>
                        {hw.length===0?<p style={{fontSize:13,color:"#CCC",margin:0}}>등록된 숙제 없음</p>:(
              ...
              <div style={{marginBottom:8}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                  <p style={{fontSize:13,color:C.sub,fontWeight:700,margin:0,letterSpacing:0.5}}>📋 등록 학원 ({curAc.length})</p>
                  <button onClick={openAdd} style={{fontSize:12,padding:"6px 14px",borderRadius:8,border:"none",background:th.grad,color:"#fff",fontWeight:700,cursor:"pointer"}}>+ 학원 추가</button>
                </div>
                {curAc.length===0?(
                  <div style={{textAlign:"center",padding:"28px",color:C.sub,fontSize:14,background:C.card,borderRadius:14,border:`1.5px dashed ${C.border}`}}>
                      ...
                      <div key={ac.id} style={{background:C.card,borderRadius:14,border:`1.5px solid ${ac.color}30`,overflow:"hidden",boxShadow:`0 2px 10px ${ac.color}10`}}>
                        <div style={{background:`${ac.color}10`,padding:"12px 16px",display:"flex",alignItems:"center",gap:10}}>
                          ...
                          <div style={{flex:1}}>
                            ...
                            <p style={{fontSize:12,color:C.sub,margin:"3px 0 0"}}>{ac.days.join("·")}요일 &nbsp;·&nbsp; {ac.time} &nbsp;·&nbsp; {ac.duration}분</p>
                          </div>
                          <button onClick={()=>openEdit(ac)} style={{padding:"6px 12px",borderRadius:8,border:`1px solid ${ac.color}40`,background:`${ac.color}10`,color:ac.color,fontSize:12,fontWeight:700,cursor:"pointer",flexShrink:0}}>✏️ 수정</button>
                        </div>
                        <div style={{padding:"10px 16px",display:"flex",alignItems:"center",gap:10}}>
                          <div style={{flex:1,display:"flex",flexWrap:"wrap",gap:8}}>
                            <span style={{fontSize:12,color:C.sub}}>💰 월 {Number(ac.fee).toLocaleString()}원</span>
                            {ac.teacher&&<span style={{fontSize:12,color:C.sub}}>👩‍🏫 {ac.teacher}</span>}
                          </div>
                          <div style={{display:"flex",gap:6,flexShrink:0}}>
                            {ac.phone&&<a href={`tel:${ac.phone}`} style={{width:34,height:34,borderRadius:8,background:`${C.green}12`,border:`1px solid ${C.green}30`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,textDecoration:"none"}}>📞</a>}
                            <button onClick={()=>{ setShowSmsModal(ac); setSmsDraft(""); }} style={{width:34,height:34,borderRadius:8,background:C.purpleL,border:`1px solid ${C.purple}30`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,cursor:"pointer"}}>💬</button>
                            <button onClick={()=>setShowDetailModal(ac)} style={{width:34,height:34,borderRadius:8,background:C.faint,border:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,cursor:"pointer"}}>›</button>
                          </div>
                        </div>
                      </div>
                    ))}
            ...
            <div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",textAlign:"center",marginBottom:4}}>
                {["월","화","수","목","금","토","일"].map((d,i)=>(
                  <div key={d} style={{fontSize:12,fontWeight:700,color:i===5?"#3498DB":i===6?"#E74C3C":C.sub,padding:"5px 0"}}>{d}</div>
                ))}
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3}}>
                {calDays.map((day,i)=>{
                  ...
                  if(pendingHwD) badges.push("⚠️");
                  else if(doneHwD) badges.push("✓");
                  ...
                  return (
                    <div key={i} onClick={()=>setCalSelDate(isSel?null:dateStr)}
                      ...
                      <div style={{fontSize:13,fontWeight:isToday||isSel?900:600,color:isToday?"#fff":isSel?th.main:dn==="일"?"#E74C3C":dn==="토"?"#3498DB":C.text,textAlign:"right",paddingRight:3,marginBottom:2}}>{day}</div>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:10,padding:"10px 12px",background:C.card,borderRadius:10,border:`1px solid ${C.border}`}}>
                {[{icon:"●",label:"학원"},{icon:"🏥",label:"결석"},{icon:"📚",label:"보충예정"},{icon:"✅",label:"보충완료"},{icon:"⚠️",label:"숙제미완"},{icon:"🎒",label:"추가준비물"},{icon:"📝",label:"메모"}].map((l,i)=>(
                  <span key={i} style={{display:"flex",alignItems:"center",gap:3,fontSize:11,color:C.sub}}>
                    <span style={{fontSize:i===0?8:11,color:i===0?th.main:"inherit"}}>{l.icon}</span>{l.label}
                  </span>
                ))}
              </div>
                  ...
                  <div style={{padding:"16px 16px"}}>
                    ...
                    {selInfo.absOnDay.length>0&&(
                      <div style={{background:`${C.red}08`,border:`1px solid ${C.red}25`,borderRadius:12,padding:"12px 14px",marginBottom:12}}>
                        <p style={{fontSize:13,fontWeight:700,color:C.red,margin:"0 0 8px"}}>🏥 결석</p>
                        {selInfo.absOnDay.map(ab=>{
                          ...
                          return (
                            <div key={ab.id} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderTop:`1px solid ${C.red}15`}}>
                              ...
                              <div style={{flex:1}}>
                                <span style={{fontSize:14,fontWeight:700,color:C.text}}>{ac.name}</span>
                                {ab.reason&&<span style={{fontSize:12,color:C.sub,marginLeft:6}}>· {ab.reason}</span>}
                              </div>
                              {ac.phone&&<button onClick={()=>{ setShowSmsModal(ac); setSmsDraft(""); }} style={{fontSize:12,padding:"4px 10px",borderRadius:7,border:`1px solid ${C.purple}30`,background:C.purpleL,color:C.purple,cursor:"pointer",fontWeight:600}}>💬 문자</button>}
                            </div>
                          );
                        })}
                      </div>
                    ...
                    {selInfo.makeupOnDay.length>0&&(
                      <div style={{background:`${C.orange}08`,border:`1px solid ${C.orange}30`,borderRadius:12,padding:"12px 14px",marginBottom:12}}>
                        <p style={{fontSize:13,fontWeight:700,color:C.orange,margin:"0 0 8px"}}>📚 보충수업</p>
                        {selInfo.makeupOnDay.map(ab=>{
                          ...
                          return (
                            <div key={ab.id} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderTop:`1px solid ${C.orange}15`}}>
                              ...
                              <div style={{flex:1}}>
                                <span style={{fontSize:14,fontWeight:700,color:C.text}}>{ac.name}</span>
                                <p style={{fontSize:12,color:C.sub,margin:"2px 0 0"}}>결석일: {ab.date}</p>
                              </div>
                              <button onClick={()=>setAbsences(p=>({...p,[childId]:(p[childId]||[]).map(a=>a.id===ab.id?{...a,makeupDone:!a.makeupDone}:a)}))}
                                style={{fontSize:12,padding:"4px 10px",borderRadius:7,border:`1px solid ${ab.makeupDone?C.green+"40":C.orange+"40"}`,background:ab.makeupDone?`${C.green}12`:`${C.orange}12`,color:ab.makeupDone?C.green:C.orange,cursor:"pointer",fontWeight:700}}>
                    ...
                    <div style={{marginBottom:14}}>
                      <p style={{fontSize:12,fontWeight:700,color:C.sub,margin:"0 0 7px",letterSpacing:0.5}}>📝 날짜 메모</p>
                      <div style={{display:"flex",gap:8}}>
                        <input value={dayMemos[selInfo.mk]||""} onChange={e=>setDayMemos(p=>({...p,[selInfo.mk]:e.target.value}))}
                          placeholder="메모 입력..."
                          style={{flex:1,background:C.faint,border:`1px solid ${C.faintB}`,borderRadius:9,padding:"9px 12px",fontSize:14,color:C.text,outline:"none"}}/>
                        ...
                        <div key={ac.id} style={{marginBottom:12,borderRadius:14,border:`1.5px solid ${ac.color}25`,overflow:"hidden"}}>
                          <div style={{background:`${ac.color}10`,padding:"10px 14px",display:"flex",alignItems:"center",gap:10}}>
                            ...
                            <div style={{flex:1}}>
                              ...
                              <p style={{fontSize:12,color:C.sub,margin:"2px 0 0"}}>{ac.time} ~ {endT}</p>
                            </div>
                            {hw.length>0&&<span style={{fontSize:11,fontWeight:700,color:allDone?C.green:C.orange,background:allDone?`${C.green}15`:`${C.orange}15`,borderRadius:6,padding:"3px 8px"}}>{allDone?"✓ 완료":`${doneCnt}/${hw.length}`}</span>}
                          </div>
                          <div style={{padding:"12px 14px"}}>
                            <div style={{marginBottom:10}}>
                              <p style={{fontSize:12,fontWeight:700,color:C.sub,margin:"0 0 6px"}}>🎒 준비물</p>
                              <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                                {(ac.baseSupplies||[]).map((s,i)=><span key={`b${i}`} style={{fontSize:12,padding:"3px 10px",borderRadius:20,background:`${ac.color}18`,color:ac.color,fontWeight:600}}>{s}</span>)}
                                {sup.map((s,i)=><span key={`d${i}`} style={{fontSize:12,padding:"3px 10px",borderRadius:20,background:`${C.orange}15`,color:C.orange,fontWeight:600}}>+{s}</span>)}
                                {(ac.baseSupplies||[]).length===0&&sup.length===0&&<span style={{fontSize:12,color:"#CCC"}}>없음</span>}
                              </div>
                            </div>
                            <div style={{marginBottom:10}}>
                              <p style={{fontSize:12,fontWeight:700,color:C.sub,margin:"0 0 6px"}}>📝 숙제</p>
                              {hw.length===0?<p style={{fontSize:13,color:"#CCC",margin:0}}>없음</p>:(
                            ...
                            <button onClick={()=>{ setShowDailyModal({academyId:ac.id,date:calSelDate,acName:ac.name,acColor:ac.color,baseSupplies:ac.baseSupplies}); setDailyHwInput(""); setDailySupInput(""); }}
                              style={{width:"100%",padding:"8px",borderRadius:9,border:`1.5px dashed ${ac.color}50`,background:`${ac.color}06`,color:ac.color,fontSize:13,fontWeight:700,cursor:"pointer"}}>
                              ✏️ 숙제 · 준비물 편집
                            </button>
                          </div>
                        </div>
              ...
              {!selInfo&&<div style={{marginTop:12,textAlign:"center",padding:"18px",color:C.sub,fontSize:14,background:C.card,borderRadius:12,border:`1.5px dashed ${C.border}`}}>날짜를 탭하면 학원·숙제·결석·보충수업을 확인할 수 있어요</div>}
            </div>
          );
        })()}
        ...
        {tab==="fee"&&(
          <div>
            ...
            <div style={{background:th.grad,borderRadius:16,padding:"20px 22px",marginBottom:16,color:"#fff"}}>
              <p style={{fontSize:12,opacity:0.8,margin:0,fontWeight:600}}>{th.emoji} {curChild?.name} 총 학원비</p>
              <p style={{fontSize:32,fontWeight:900,margin:"6px 0 4px"}}>{totalFee(childId).toLocaleString()}원</p>
              <p style={{fontSize:13,opacity:0.75,margin:0}}>납부 {curAc.filter(a=>isPaid(a.id)).length}/{curAc.length}개 완료</p>
            </div>
            {curAc.map(a=>{
              ...
              return (
                <div key={a.id} style={{background:C.card,borderRadius:14,padding:"16px 18px",marginBottom:12,border:`1px solid ${isPaid(a.id)?C.green+"40":C.border}`}}>
                  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                    ...
                    <p style={{fontSize:16,fontWeight:700,margin:0,flex:1,color:C.text}}>{a.name}</p>
                    <button onClick={()=>togglePaid(a.id)} style={{padding:"6px 14px",borderRadius:8,border:"none",cursor:"pointer",fontSize:13,fontWeight:700,background:isPaid(a.id)?`${C.green}18`:C.faint,color:isPaid(a.id)?C.green:C.sub}}>
                      {isPaid(a.id)?"✓ 납부완료":"미납"}
                    </button>
                  </div>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div style={{display:"flex",gap:20}}>
                      <div><p style={{fontSize:11,color:C.sub,margin:0}}>월 학원비</p><p style={{fontSize:17,fontWeight:800,margin:"3px 0 0",color:C.text}}>{Number(a.fee).toLocaleString()}원</p></div>
                      <div><p style={{fontSize:11,color:C.sub,margin:0}}>납부일</p><p style={{fontSize:17,fontWeight:800,margin:"3px 0 0",color:C.text}}>매월 {a.payDay}일</p></div>
                    </div>
                    <span style={{fontSize:13,fontWeight:700,padding:"5px 12px",borderRadius:8,background:`${st.color}15`,color:st.color}}>{st.label}</span>
                  </div>
                </div>
              );
            })}
            ...
            <div style={{display:"flex",gap:8,marginBottom:16}}>
              {[{l:"전체",v:curAbs.length,c:C.red},{l:"보충 예정",v:curAbs.filter(a=>a.makeupDate&&!a.makeupDone).length,c:C.orange},{l:"보충 완료",v:curAbs.filter(a=>a.makeupDone).length,c:C.green}].map((s,i)=>(
                <div key={i} style={{flex:1,background:C.card,borderRadius:12,padding:"14px 8px",textAlign:"center",border:`1px solid ${s.c}20`}}>
                  <p style={{fontSize:11,color:C.sub,margin:0}}>{s.l}</p>
                  <p style={{fontSize:22,fontWeight:800,margin:"4px 0 0",color:s.c}}>{s.v}</p>
                </div>
              ))}
            </div>
            <button onClick={()=>setShowAbsModal(true)} style={{width:"100%",padding:14,borderRadius:12,border:`1.5px dashed ${C.red}55`,background:`${C.red}08`,color:C.red,fontSize:15,fontWeight:700,cursor:"pointer",marginBottom:16}}>+ 결석 기록 추가</button>
                  ...
                  <div style={{display:"flex",gap:10}}>
                    ...
                    <div style={{flex:1}}>
                      <div style={{display:"flex",justifyContent:"space-between"}}>
                        <p style={{fontWeight:700,fontSize:16,margin:0,color:C.text}}>{ac.name}</p>
                        <button onClick={()=>deleteAbs(ab.id)} style={{background:"none",border:"none",color:"#CCC",cursor:"pointer",fontSize:18}}>✕</button>
                      </div>
                      <p style={{fontSize:13,color:C.sub,margin:"4px 0 10px"}}>결석일: {ab.date}{ab.reason&&` · ${ab.reason}`}</p>
                      <div style={{padding:"12px 14px",borderRadius:10,background:ab.makeupDone?`${C.green}0D`:past?`${C.red}0D`:C.faint,border:`1px solid ${ab.makeupDone?C.green+"33":past?C.red+"33":C.faintB}`}}>
                        {ab.makeupDate?(
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                            <div>
                              <p style={{fontSize:11,color:C.sub,margin:0}}>보충 일정</p>
                              <p style={{fontSize:14,fontWeight:700,margin:"3px 0 0",color:ab.makeupDone?C.green:past?C.red:C.text}}>{ab.makeupDate}</p>
                              {past&&!ab.makeupDone&&<p style={{fontSize:11,color:C.red,margin:"2px 0 0"}}>⚠️ 보충일이 지났어요</p>}
                            </div>
                            <button onClick={()=>toggleMakeup(ab.id)} style={{padding:"7px 14px",borderRadius:8,border:"none",cursor:"pointer",fontSize:13,fontWeight:700,background:ab.makeupDone?`${C.green}18`:C.faint,color:ab.makeupDone?C.green:C.sub}}>{ab.makeupDone?"✓ 완료":"완료 처리"}</button>
                          </div>
                        ):<p style={{fontSize:13,color:C.sub,margin:0}}>📭 보충 일정 미정</p>}
                      </div>
                      <button onClick={()=>{ setShowSmsModal(ac); setSmsDraft(""); }} style={{width:"100%",marginTop:10,padding:"9px",borderRadius:10,border:`1px solid ${C.purple}30`,background:C.purpleL,color:C.purple,fontSize:13,fontWeight:700,cursor:"pointer"}}>💬 결석 안내 문자 보내기</button>
                    </div>
                  </div>
                </div>
              );
            })}
            {curAbs.length===0&&<div style={{textAlign:"center",padding:"40px 20px",background:C.card,borderRadius:16,border:`1.5px dashed ${C.border}`}}><p style={{fontSize:32,margin:0}}>🙌</p><p style={{color:C.sub,fontSize:15,margin:"8px 0 0"}}>결석 기록이 없어요!</p></div>}
          </div>
        )}
        ...
        {tab==="sms"&&(
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              ...
              <button onClick={()=>{ setShowTmplEdit("new"); setEditTmpl({title:"",body:""}); }} style={{padding:"8px 16px",borderRadius:10,border:"none",background:th.grad,color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer"}}>+ 새 템플릿</button>
            </div>
            <div style={{background:`${C.purple}08`,border:`1px solid ${C.purple}25`,borderRadius:12,padding:"12px 16px",marginBottom:16}}>
              <p style={{fontSize:12,color:C.purple,fontWeight:700,margin:"0 0 6px"}}>📌 사용 가능한 변수</p>
              <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                {["{아이이름}","{학원명}","{날짜}","{시간}"].map(v=><span key={v} style={{fontSize:12,padding:"3px 10px",borderRadius:6,background:C.purpleL,color:C.purple,fontWeight:600}}>{v}</span>)}
              </div>
            </div>
            {templates.map(tmpl=>(
              <div key={tmpl.id} style={{background:C.card,borderRadius:14,padding:"16px 18px",marginBottom:12,border:`1px solid ${C.border}`}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                  <span style={{fontWeight:700,fontSize:15,color:C.text}}>💬 {tmpl.title}</span>
                  <div style={{display:"flex",gap:6}}>
                    <button onClick={()=>{ setShowTmplEdit(tmpl.id); setEditTmpl({title:tmpl.title,body:tmpl.body}); }} style={{padding:"5px 12px",borderRadius:7,border:`1px solid ${C.border}`,background:C.faint,color:C.sub,fontSize:12,cursor:"pointer"}}>수정</button>
                    <button onClick={()=>{ setTemplates(p=>p.filter(t=>t.id!==tmpl.id)); showToast("삭제됨"); }} style={{padding:"5px 12px",borderRadius:7,border:`1px solid ${C.red}30`,background:`${C.red}0A`,color:C.red,fontSize:12,cursor:"pointer"}}>삭제</button>
                  </div>
                </div>
                <p style={{fontSize:13,color:C.sub,margin:0,whiteSpace:"pre-wrap",background:C.faint,borderRadius:8,padding:"10px 12px"}}>{tmpl.body}</p>
              </div>
            ))}
          </div>
        )}
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(20,20,40,0.55)",display:"flex",alignItems:"flex-end",zIndex:300}} onClick={()=>{ setShowChildMgr(false); setEditingChild(null); }}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"22px 22px 0 0",padding:"24px 20px 48px",width:"100%",maxWidth:430,maxHeight:"90vh",overflowY:"auto",boxSizing:"border-box"}}>
            ...
            <div style={{background:GENDER_THEME[childForm.gender].grad,borderRadius:12,padding:"14px 18px",marginBottom:24,color:"#fff",textAlign:"center"}}>
              ...
              <p style={{fontSize:15,fontWeight:700,margin:0}}>{childForm.name||"이름 미입력"}</p>
            </div>

            <button onClick={saveChild} style={{width:"100%",padding:15,borderRadius:14,border:"none",background:GENDER_THEME[childForm.gender].grad,color:"#fff",fontSize:16,fontWeight:700,cursor:"pointer",boxShadow:`0 4px 16px ${GENDER_THEME[childForm.gender].main}40`}}>
            ...
            {!editingChild&&children.length>0&&(
              <div style={{marginTop:24,borderTop:`1px solid ${C.border}`,paddingTop:18}}>
                <p style={{fontSize:14,fontWeight:700,color:C.sub,margin:"0 0 12px"}}>등록된 아이 ({children.length})</p>
                {children.map(c=>{
                  const t=GENDER_THEME[c.gender]||GENDER_THEME.boy;
                  return (
                    <div key={c.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",borderRadius:12,border:`1px solid ${C.border}`,marginBottom:8,background:C.faint}}>
                      ...
                      <span style={{flex:1,fontSize:15,fontWeight:700,color:C.text}}>{c.name}</span>
                      <button onClick={()=>{ setEditingChild(c.id); setChildForm({name:c.name,gender:c.gender}); }} style={{padding:"5px 12px",borderRadius:8,border:`1px solid ${C.border}`,background:"#fff",color:C.sub,fontSize:12,cursor:"pointer"}}>수정</button>
                      <button onClick={()=>deleteChild(c.id)} style={{padding:"5px 12px",borderRadius:8,border:`1px solid ${C.red}30`,background:`${C.red}0A`,color:C.red,fontSize:12,cursor:"pointer"}}>삭제</button>
                    </div>
                  );
                })}
              </div>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(20,20,40,0.5)",display:"flex",alignItems:"flex-end",zIndex:200}} onClick={()=>setShowAddAcModal(false)}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"22px 22px 0 0",padding:"24px 20px 48px",width:"100%",maxWidth:430,maxHeight:"93vh",overflowY:"auto",boxSizing:"border-box"}}>
            ...
            <div style={{display:"flex",gap:5,marginBottom:16}}>
              {DAYS.map(day=>(
                <button key={day} onClick={()=>toggleDay(day)} style={{flex:1,padding:"9px 0",borderRadius:8,border:`1.5px solid ${newAc.days.includes(day)?DAY_COLORS[day]:C.faintB}`,background:newAc.days.includes(day)?DAY_COLORS[day]:C.faint,color:newAc.days.includes(day)?"#fff":C.sub,fontSize:13,fontWeight:600,cursor:"pointer"}}>{day}</button>
              ))}
            </div>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:8}}>
              {(newAc.baseSupplies||[]).map((s,i)=>(
                <span key={i} style={{fontSize:13,padding:"5px 12px",borderRadius:20,background:`${newAc.color}18`,color:newAc.color,display:"flex",alignItems:"center",gap:4,fontWeight:600}}>
                  {s}<button onClick={()=>setNewAc(p=>({...p,baseSupplies:p.baseSupplies.filter((_,j)=>j!==i)}))} style={{background:"none",border:"none",color:newAc.color,cursor:"pointer",fontSize:11,padding:0}}>✕</button>
                </span>
              ))}
            </div>
            <div style={{display:"flex",gap:8,marginBottom:16}}>
              ...
              <button onClick={addBaseSupply} style={{padding:"0 18px",borderRadius:10,border:"none",background:newAc.color,color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer"}}>추가</button>
            </div>
            <div style={{borderTop:`1px solid ${C.border}`,paddingTop:16,marginBottom:16}}>
              <p style={{fontSize:14,fontWeight:700,color:C.text,margin:"0 0 14px"}}>📋 연락처 정보 <span style={{fontSize:12,color:C.sub,fontWeight:400}}>(선택)</span></p>
            ...
            <button onClick={saveAcademy} style={{width:"100%",padding:15,borderRadius:14,border:"none",background:th.grad,color:"#fff",fontSize:16,fontWeight:700,cursor:"pointer",boxShadow:`0 4px 16px ${th.main}40`}}>
          ...
          <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:22,padding:24,width:"100%",maxWidth:390,boxShadow:"0 20px 60px rgba(0,0,0,0.18)",maxHeight:"88vh",overflowY:"auto"}}>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:18}}>
              ...
              <div style={{flex:1}}>
                ...
                <p style={{margin:"3px 0 0",fontSize:13,color:C.sub}}>{showDetailModal.days.join(", ")}요일 · {showDetailModal.time} ({showDetailModal.duration}분)</p>
              </div>
            </div>
            {[["💰 월 학원비",`${Number(showDetailModal.fee||0).toLocaleString()}원`],["📆 납부일",`매월 ${showDetailModal.payDay}일`],["🎒 기본 준비물",(showDetailModal.baseSupplies||[]).join(", ")||"없음"],...(showDetailModal.teacher?[["👩‍🏫 선생님",showDetailModal.teacher]]:[]),...(showDetailModal.address?[["📍 주소",showDetailModal.address]]:[])].map(([k,v])=>(
              <div key={k} style={{display:"flex",justifyContent:"space-between",padding:"10px 0",borderBottom:`1px solid ${C.border}`}}>
                <span style={{fontSize:14,color:C.sub,flexShrink:0}}>{k}</span>
                <span style={{fontSize:14,fontWeight:600,color:C.text,textAlign:"right",maxWidth:"58%",marginLeft:8}}>{v}</span>
              </div>
            ))}
            {showDetailModal.memo&&(
              <div style={{background:`${C.orange}0D`,borderRadius:10,padding:"12px 14px",marginTop:12,border:`1px solid ${C.orange}30`}}>
                <p style={{fontSize:12,color:C.orange,margin:"0 0 4px",fontWeight:600}}>📝 메모</p>
                <p style={{fontSize:14,color:C.text,margin:0}}>{showDetailModal.memo}</p>
              </div>
            )}
            {showDetailModal.phone&&(
              <div style={{display:"flex",gap:8,marginTop:16,padding:"14px 0",borderTop:`1px solid ${C.border}`}}>
                <a href={`tel:${showDetailModal.phone}`} style={{flex:1,padding:12,borderRadius:11,background:`${C.green}12`,border:`1px solid ${C.green}30`,color:C.green,fontSize:14,fontWeight:700,textAlign:"center",textDecoration:"none",display:"block"}}>📞 {showDetailModal.phone}</a>
                <button onClick={()=>{ setShowSmsModal(showDetailModal); setShowDetailModal(null); setSmsDraft(""); }} style={{flex:1,padding:12,borderRadius:11,border:`1px solid ${C.purple}44`,background:C.purpleL,color:C.purple,fontSize:14,fontWeight:700,cursor:"pointer"}}>💬 문자 보내기</button>
              </div>
            )}
            <div style={{display:"flex",gap:8,marginTop:showDetailModal.phone?8:16}}>
              <button onClick={()=>openEdit(showDetailModal)} style={{flex:1,padding:12,borderRadius:11,border:`1px solid ${th.main}44`,background:th.light,color:th.main,fontSize:14,fontWeight:700,cursor:"pointer"}}>✏️ 수정</button>
              <button onClick={()=>deleteAcademy(showDetailModal.id)} style={{flex:1,padding:12,borderRadius:11,border:`1px solid ${C.red}44`,background:`${C.red}0D`,color:C.red,fontSize:14,fontWeight:600,cursor:"pointer"}}>🗑 삭제</button>
            </div>
          </div>
        </div>
      )}
            ...
            <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"22px 22px 0 0",padding:"24px 20px 48px",width:"100%",maxWidth:430,maxHeight:"88vh",overflowY:"auto",boxSizing:"border-box"}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:18}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <p style={{fontSize:13,color:C.sub,margin:"2px 0 0"}}>{fmt(date)} {date===TODAY?"(오늘)":""}</p>
                </div>
              ...
              {(baseSupplies||[]).length>0&&(
                <div style={{background:`${acColor}08`,borderRadius:10,padding:"10px 14px",marginBottom:16}}>
                  <p style={{fontSize:12,color:acColor,margin:"0 0 6px",fontWeight:700}}>🎒 항상 챙길 준비물</p>
                  <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                    {baseSupplies.map((s,i)=><span key={i} style={{fontSize:12,padding:"3px 10px",borderRadius:20,background:acColor,color:"#fff",fontWeight:600}}>{s}</span>)}
                  </div>
                </div>
              )}
              <p style={{fontSize:14,fontWeight:700,color:C.text,margin:"0 0 10px"}}>📝 이번 수업 숙제</p>
              {hw.length===0&&<p style={{fontSize:13,color:C.sub,marginBottom:10}}>등록된 숙제가 없어요</p>}
              ...
              <div style={{display:"flex",gap:8,marginBottom:20}}>
                <input value={dailyHwInput} onChange={e=>setDailyHwInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addHw()} placeholder="숙제 입력 후 Enter" style={{...inp,flex:1,width:"auto",fontSize:14,padding:"10px 14px"}}/>
                <button onClick={addHw} style={{padding:"0 18px",borderRadius:10,border:"none",background:acColor,color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer"}}>추가</button>
              </div>
              <p style={{fontSize:14,fontWeight:700,color:C.text,margin:"0 0 10px"}}>📦 이번 수업 추가 준비물</p>
              <div style={{display:"flex",flexWrap:"wrap",gap:7,marginBottom:10}}>
                {sup.map((s,i)=>(
                  <span key={i} style={{fontSize:13,padding:"5px 14px",borderRadius:20,background:`${acColor}15`,color:acColor,display:"flex",alignItems:"center",gap:5,fontWeight:600}}>
                ...
                {sup.length===0&&<p style={{fontSize:13,color:C.sub,margin:0}}>추가 준비물 없음</p>}
              </div>
              <div style={{display:"flex",gap:8,marginBottom:22}}>
                <input value={dailySupInput} onChange={e=>setDailySupInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addSup()} placeholder="추가 준비물 입력 후 Enter" style={{...inp,flex:1,width:"auto",fontSize:14,padding:"10px 14px"}}/>
                <button onClick={addSup} style={{padding:"0 18px",borderRadius:10,border:"none",background:acColor,color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer"}}>추가</button>
              </div>
              <button onClick={()=>{ setShowDailyModal(null); showToast(); }} style={{width:"100%",padding:15,borderRadius:14,border:"none",background:th.grad,color:"#fff",fontSize:16,fontWeight:700,cursor:"pointer"}}>저장 & 닫기</button>
            </div>
          </div>
        );
      })()}
          ...
          <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"22px 22px 0 0",padding:"24px 20px 44px",width:"100%",maxWidth:430,maxHeight:"90vh",overflowY:"auto",boxSizing:"border-box"}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:18}}>
              ...
              <div style={{flex:1}}>
                ...
                {showSmsModal.phone&&<p style={{fontSize:13,color:C.sub,margin:"2px 0 0"}}>📞 {showSmsModal.phone}</p>}
              </div>
            ...
            <p style={{fontSize:13,color:C.sub,fontWeight:700,margin:"0 0 10px"}}>📋 템플릿 선택</p>
            <div style={{display:"flex",gap:7,flexWrap:"wrap",marginBottom:16}}>
              {templates.map(t=><button key={t.id} onClick={()=>applyTmpl(t,showSmsModal)} style={{padding:"7px 14px",borderRadius:8,border:`1px solid ${C.purple}40`,background:C.purpleL,color:C.purple,fontSize:13,fontWeight:600,cursor:"pointer"}}>{t.title}</button>)}
            </div>
            ...
            <div style={{display:"flex",gap:10}}>
              {showSmsModal.phone?(
                <>
                  <a href={smsLink(showSmsModal.phone,smsDraft)} style={{flex:2,padding:14,borderRadius:14,border:"none",background:`linear-gradient(135deg,${C.purple},#9B7FFF)`,color:"#fff",fontSize:15,fontWeight:700,textAlign:"center",textDecoration:"none",display:"block",boxShadow:`0 4px 14px ${C.purple}40`}}>📲 문자 앱으로 발송</a>
                  <a href={`tel:${showSmsModal.phone}`} style={{flex:1,padding:14,borderRadius:14,border:`1.5px solid ${C.green}40`,background:`${C.green}10`,color:C.green,fontSize:15,fontWeight:700,textAlign:"center",textDecoration:"none",display:"block"}}>📞 전화</a>
                </>
              ):<p style={{fontSize:14,color:C.red,margin:0}}>⚠️ 연락처가 등록되지 않았어요</p>}
            </div>
          </div>
        </div>
      )}
      ...
      {showTmplEdit&&(
        <div style={{position:"fixed",inset:0,background:"rgba(20,20,40,0.5)",display:"flex",alignItems:"flex-end",zIndex:300}} onClick={()=>setShowTmplEdit(null)}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"22px 22px 0 0",padding:"24px 20px 48px",width:"100%",maxWidth:430,boxSizing:"border-box"}}>
            ...
            <div style={{display:"flex",gap:10}}>
              <button onClick={()=>setShowTmplEdit(null)} style={{flex:1,padding:14,borderRadius:12,border:`1px solid ${C.border}`,background:C.faint,color:C.sub,fontSize:15,cursor:"pointer"}}>취소</button>
              <button onClick={saveTmpl} style={{flex:2,padding:14,borderRadius:12,border:"none",background:th.grad,color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer"}}>저장</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showAbsModal&&(
        <div style={{position:"fixed",inset:0,background:"rgba(20,20,40,0.5)",display:"flex",alignItems:"flex-end",zIndex:200}} onClick={()=>setShowAbsModal(false)}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"22px 22px 0 0",padding:"24px 20px 48px",width:"100%",maxWidth:430,boxSizing:"border-box"}}>
            ...
            <button onClick={addAbs} style={{width:"100%",padding:15,borderRadius:14,border:"none",background:`linear-gradient(135deg,${C.red},#FF8FA3)`,color:"#fff",fontSize:16,fontWeight:700,cursor:"pointer"}}>기록하기</button>
          </div>
        </div>
      )}
    </div>
