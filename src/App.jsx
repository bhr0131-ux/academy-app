              ...
              {(()=>{
                ...
                return (
                  <div style={{position:"relative",zIndex:2,marginTop:cute?10:14,display:"flex",alignItems:"center",justifyContent:cute?"center":"flex-start",gap:8,flexWrap:"wrap"}}>
                    ...
                    {/* 아바타 ↔ 성장 캐릭터 토글 — 상태별 나무 팻말 이미지 버튼, 눌림 효과(jelly-tap) 유지 */}
                    {(()=>{
                      const isAvatarMode = getCharMode(childId)===CHAR_DISPLAY_AVATAR;
                      // 아바타 표시 중이면 '성장 캐릭터'로 전환하는 팻말, 아니면 '내 아바타'로 전환하는 팻말.
                      const btn = isAvatarMode
                        ? { src:"/assets/ui/btn-growth-char.webp", label:"성장 캐릭터 보기" }
                        : { src:"/assets/ui/btn-my-avatar.webp",    label:"내 아바타 보기" };
                      return (
                        <button onClick={toggleCharDisplayMode} className="jelly-tap" aria-label={btn.label}
