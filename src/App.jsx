...
export default function App() {
  ...
  /* [사용자 확정 2026-08-09] 학원비 납부 기록 — '언제 얼마를 어떻게 냈나'.
     기존 납부 여부(v6_paid)는 그대로 두고 새 키에만 쌓는다. 기록이 없어도
     납부 여부는 예전과 똑같이 동작하므로 기존 사용자 데이터는 영향이 없다. */
  ...
  useEffect(()=>{
    ...
    DECOR_STAGE_BG_ALL.forEach(u=>{ if(u) urls.push(u); });
  ...
  useEffect(()=>{
    (async()=>{
      ...
      const RETIRED_AVATAR_ITEMS={ shoes_boots_desert:140, shoes_boots_ribbon:150, background_forest:120, background_galaxy:300, shoes_boots_green:100, back_backpack:250, shoes_picnic:180 }; // { 은퇴 아이템 id: 환불 코인 } (배경은 아바타 꾸미기에서 제거됨 — 구 꾸미기 상점 배경과 중복. background_sky는 무료라 환불 없이 normalize에서 정리)
  ...
  useEffect(()=>{ if(loaded) save("v6_fee_pay_info",payInfo); },[payInfo,loaded]);
  /* 고른 아이가 줄 밖으로 밀려 있으면 가운데로 끌어온다 (사용자 확정 2026-08-09).
     아이가 많을 때 지금 보고 있는 아이가 어디 있는지 매번 찾지 않아도 된다. */
  useEffect(()=>{
    const box=childStripRef.current; if(!box) return;
    const el=box.querySelector(`[data-child="${childId}"]`); if(!el) return;
    const to=el.offsetLeft-(box.clientWidth-el.offsetWidth)/2;
  ...
  const applyRestore=(data)=>{
    ...
    try{
        ...
        setPayInfo(data.payInfo||{});   // 구버전 백업엔 없는 값 — 없으면 빈 값
  ...
  const buyDecor=(decor)=>{
    ...
    /* [2026-09-26] 펫 스킨 잠금은 화면(DecorShopSheet)에만 있었다 — 잠긴 그룹은 카드를
       아예 안 그려서 지금은 누를 버튼이 없지만, 화면을 고치다 노출되면 그냥 사진다.
       코인을 빼기 전에 여기서도 막는다. */
    if(decorNeedsMaxPet(decor.id) && !isMaxPet(cid)){
      showToast(kidSkin==="cute"?"펫이 마지막까지 자라면 열려요 🐾":"펫이 마지막까지 진화하면 열려요 🐾");
      return;
    }
  ...
  /* 납부 기록(언제·얼마·어떻게). 없을 수도 있다 — 예전 방식으로 납부만 체크한 건은 기록이 없다. */
  const payRec=(aId)=>payInfo[pKey(childId,aId)]||null;
  const savePayRec=(aId,rec)=>{
    const k=pKey(childId,aId);
    setPayInfo(p=>({...p,[k]:rec}));
    setPaidStatus(p=>({...p,[k]:true}));
  };
  const clearPayRec=(aId)=>{
    const k=pKey(childId,aId);
    setPayInfo(p=>{ const n={...p}; delete n[k]; return n; });
    setPaidStatus(p=>({...p,[k]:false}));
  };
  /* [사용자 확정 2026-08-09] 상태 이름을 한 벌로 통일한다.
     납부 예정(회색) → D-2(주황) → 오늘 납부일(주황) → 납부일 N일 지남(코랄) → 납부 완료(민트).
     '연체'라고 쓰지 않는 이유: 앱은 달력의 납부일만 보고 판단할 뿐,
          ...
          {childTab==="area"&&(
            <>
              ...
              {(()=>{
                ...
                if(kidSkin!=="cute"){
                  /* [사용자 확정 2026-09-26] 학원이 없는 날에도 지도를 보여 준다.
                     예전엔 0곳이면 지도를 통째로 안 그려서, 학원을 안 다니는 아이는
                     탐험 탭에 '쉬는 날' 상자 하나만 있었다 — 탐험 탭인데 탐험할 게 없었다.
                     지도는 건물만 없을 뿐 길·동물·보물상자가 그대로 있어서 그림책처럼 남는다.
                     (아래 '오늘은 탐험 장소가 없어요' 상자는 그대로 둔다 — 일정이 없다는 안내다) */
                  return (
                    <>
                    ...
                    <div style={{margin:"-4px -16px 20px"}}>
                      <AdventureMap
                        ...
                        spark={(()=>{
                          ...
                          /* [사용자 확정 2026-09-26] 갈 곳이 없는 날엔 발견 지점도 안 그린다.
                             학원이 0곳이면 아이가 길을 걷지 않아(targetT=0) 그 지점에 닿을 수
                             없다 — 닿지도 못할 ✨ 를 깜빡이게 두면 헛기다리게 된다.
                             같은 이유로 아래 '오늘의 발견' 힌트 줄도 0곳인 날엔 숨긴다
                             (2026-08-09 확정). 이미 받아 둔 날(_d)은 기록이라 계속 보여 준다. */
                          if(!_d && childTodayAc.length===0) return null;
      ...
      <div style={{position:"relative",margin:"-28px 14px 0",background:"#fff",borderRadius:16,boxShadow:SHADOW.lg,padding:"7px 8px",display:"flex",alignItems:"center",gap:7,zIndex:5}}>
        ...
        <div className="noscrollbar" ref={childStripRef} style={{display:"flex",flex:1,gap:5,overflowX:"auto"}}>
          {children.map(c=>{
            ...
            return (
              /* [사용자 확정 2026-08-09] 줄 높이를 한 단계 줄이고, 고른 아이의 색 면적도 줄였다.
      ...
      <div style={{padding:"22px 16px 0",paddingBottom:`calc(${PARENT_NAV_H + 100}px + env(safe-area-inset-bottom))`}}>
        ...
        {tab==="home"&&(
          <ParentHomeTab
            ...
            navH={PARENT_NAV_H}
            isVacationDay={isVacationDay} getDailyEntry={getDailyEntry}
        ...
        {/* [2026-08-09] 화면 본문을 components/parent/CalendarTab.jsx 로 옮겼다
            (CLAUDE.md 규칙 3 — 화면을 고칠 때 그 화면을 컴포넌트로 뺀다).
            데이터·저장은 여전히 여기 App에 있고, 아래로는 값과 콜백만 내려보낸다. */}
        {tab==="calendar"&&(
          <CalendarTab
        ...
        {/* ════ 학원비 탭 ════ */}
        {/* [2026-08-09] 화면 본문을 components/parent/FeeTab.jsx 로 옮겼다.
            납부 여부(v6_paid)·납부 기록(v6_fee_pay_info) 저장은 그대로 App에 남는다. */}
        {tab==="fee"&&(
          <FeeTab
            th={th} CT={CT} curAc={curAc}
            feeMonth={feeMonth} setFeeMonth={setFeeMonth}
            isPaid={isPaid} payRec={payRec} payStatus={payStatus}
                  ...
                  {(
                    <div>
                          ...
                          {rewardTodayTodos.map(item=>(
                            ...
                            <div key={`${item.kind}-${item.academyId}-${item.date}-${item.id}`} style={{display:"flex",alignItems:"flex-start",gap:4,padding:"8px 12px 8px 4px",borderRadius:RAD.md,background:item.done?`${C.green}0C`:item.failed?`${C.sub}0C`:"#fff",border:`1px solid ${item.done?C.green+"33":item.failed?C.border:C.border}`}}>
                              ...
                              {/* [사용자 확정 2026-08-11] 제목·날짜보다 미션이 먼저 보여야 한다 →
                                  XP 는 오른쪽 위가 아니라 학원·종류와 같은 아랫줄에 작은 배지로
                                  붙여 카드가 덜 흩어지게 한다.
                                  ※ 한때 17까지 키웠으나 미션 탭만 글자가 커 보인다는 지적 →
                                    다른 탭의 주요 항목(홈 15 · 학원 14.5 · 학원비 14)에 맞춰 15. */}
                              <div style={{flex:1,minWidth:0,paddingTop:6}}>
                                <p style={{fontSize:15,fontWeight:800,margin:0,lineHeight:1.35,
                      ...
                      <div style={{marginTop:12,display:"flex",flexDirection:"column",gap:8}}>
                        ...
                        {/* 학원 목록 — 버튼 아래로 펼친다 */}
                        {missionPickOpen&&(()=>{
                          /* [사용자 확정 2026-08-16] 학원마다 독립된 큰 카드로 늘어놓으니
                             '오늘 미션'보다 아래 관리 메뉴가 더 중요해 보였다. 학원이 5곳만
                             돼도 한 화면을 다 먹었다. 여기서 하는 일은 "어느 학원의 미션을
                             관리할까?" 하나뿐이므로 위계를 낮춘다.
                               · 카드 5장 → 흰 목록 패널 하나 (줄 사이는 구분선만)
                               · 학원색은 테두리·배경에서 빼고 왼쪽 점에만 남긴다
        ...
        {/* ════ 보상 탭 ════ */}
        {/* ════ 보상 탭 ════ */}
        {/* [분리 2026-08-12] 화면은 components/parent/RewardTab.jsx 가 그린다 (CLAUDE.md 3).
            상태·게임 로직은 여기 App 에 그대로 두고, 필요한 값·함수만 D 하나로 넘긴다
      ...
      {showPastMissionModal&&(()=>{
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(20,20,40,0.55)",display:"flex",alignItems:"flex-end",zIndex:1000}} onClick={()=>setShowPastMissionModal(null)}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"22px 22px 0 0",padding:"22px 18px 44px",width:"100%",maxWidth:430,boxSizing:"border-box",maxHeight:"82vh",overflowY:"auto"}}>
            ...
            {cands.length===0?(
              /* [사용자 확정 2026-08-11] 이모지 폭죽 대신 앱이 쓰는 선 아이콘으로 —
                 기기마다 모양이 달라지지 않고 다른 화면의 빈 상태와 톤이 맞는다. */
              <div style={{textAlign:"center",padding:"28px 10px",color:C.sub}}>
                <span style={{display:"inline-flex",alignItems:"center",justifyContent:"center",width:52,height:52,borderRadius:"50%",background:`${C.green}14`,color:C.green}}>
                  <CareIcon name="makeup" size={26}/>
                </span>
        ...
        return (
          <div style={{position:"fixed",inset:0,background:"rgba(20,20,40,0.5)",display:"flex",alignItems:"flex-end",zIndex:300}} onClick={()=>setShowDailyModal(null)}>
            <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"22px 22px 0 0",padding:"24px 20px 48px",width:"100%",maxWidth:430,maxHeight:"88vh",overflowY:"auto",boxSizing:"border-box"}}>
              ...
              {/* [사용자 확정 2026-08-11] '숙제 입력 / 10 / 점 / [숙제]' 줄이 둘로 나뉘어 있어
                  마지막 주황 버튼이 무슨 일을 하는지 한 번 생각해야 했다 →
                  종류를 먼저 고르고, 내용과 보상을 넣고, '＋ 추가'로 끝내는 한 묶음으로 합친다. */}
              {(()=>{
                const kind=isExtra?"todo":dailyKind;
                const val =kind==="hw"?dailyHwInput:dailyTodoInput;
                const setVal=kind==="hw"?setDailyHwInput:setDailyTodoInput;
                const pt  =kind==="hw"?dailyHwPoint:dailyTodoPoint;
