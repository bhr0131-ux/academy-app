...
/* ── 학원비 항목 추가 고르기 ────────────────────────────────────────────
   [사용자 확정 2026-08-09] 학원비 탭에서 '＋ 학원비 항목 추가'를 누르면 연다.
   학원비는 학원에 딸린 값이라 '항목'을 따로 만들 수 없다 — 그래서 아직 학원비를
   안 넣은 학원을 골라 넣게 하고, 고를 학원이 없으면 학원 등록으로 보낸다.

   props
     list     : [{id,name,color,kindLabel}]  학원비가 없는 학원들
     onPick   : (acId)=>void
     onAddAc  : ()=>void        새 학원 등록으로 보내기
     onClose  : ()=>void
     tone     : FeePaySheet와 같은 색 토큰                                   */
export function FeeAddPicker({ list = [], onPick, onAddAc, onClose, tone }) {
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(20,20,40,0.55)",
      display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 1000 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: "22px 22px 0 0",
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
          <h3 style={{ margin: 0, fontSize: 17, fontWeight: 900, color: tone.text }}>학원비 항목 추가</h3>
          <button onClick={onClose} aria-label="닫기" className="jelly-tap"
        ...
        <p style={{ fontSize: 12, color: tone.sub, fontWeight: 600, margin: "0 0 14px" }}>
          {list.length > 0
            ? "학원비를 넣을 학원을 고르세요."
            : "등록된 학원에 이미 학원비가 다 들어 있어요."}
        </p>

        <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
          {list.map(ac => (
            <button key={ac.id} onClick={() => onPick(ac.id)} className="jelly-tap"
              style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", textAlign: "left",
                padding: "11px 13px", borderRadius: 12, cursor: "pointer", fontFamily: F,
                border: `1px solid ${tone.border}`, background: "#fff" }}>
              <span style={{ width: 4, height: 22, borderRadius: 10, background: ac.color, flexShrink: 0 }} />
              ...
              {ac.kindLabel && <span style={{ flexShrink: 0, fontSize: 11.5, fontWeight: 700, color: tone.sub }}>{ac.kindLabel}</span>}
            </button>
          ))}
        </div>

        <button onClick={onAddAc} className="jelly-tap"
          style={{ width: "100%", marginTop: list.length ? 12 : 0, padding: "11px 10px", borderRadius: 12,
            border: `1.5px dashed ${tone.main}55`, background: "#fff", color: tone.main,
            fontSize: 13.5, fontWeight: 800, cursor: "pointer", fontFamily: F }}>
          ＋ 새 학원 등록하기
        </button>
      </div>
    </div>
