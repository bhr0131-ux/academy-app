    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 4000,
      ...
      <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 460, height: "88vh",
        ...
        <div style={{ padding: "16px 20px 14px",
          background: dark ? "linear-gradient(160deg, #7398A8, #648492)" : "linear-gradient(160deg,#FDE7EF,#F9C5D6)",
        ...
        <div style={{ padding: "14px 14px 26px", overflowY: "auto", WebkitOverflowScrolling: "touch" }}>
          {list.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 10px" }}>
              ...
              <p style={{ fontSize: 15, fontWeight: 900, color: dark ? "#EAF0FF" : C.text, margin: "0 0 6px" }}>
                아직 {logName}이 없어요
              </p>
              <p style={{ fontSize: 13, color: dark ? "rgba(255,255,255,0.82)" : C.sub, margin: 0 }}>
                미션을 완료하면 기록이 쌓여요
              </p>
            </div>
          ) : (
                ...
                return (
                  <div key={item.id} style={dark
                    ? { display: "flex", gap: 12, alignItems: "center", padding: "12px", paddingLeft: 14, borderRadius: 14,
                        background: "linear-gradient(135deg, #28324F 0%, #323E60 100%)", border: "1px solid rgba(150,175,225,0.16)",
                        borderLeft: `4px solid ${bar}`, boxShadow: "0 3px 10px rgba(8,16,40,0.34)", marginBottom: 8 }
                    ...
                    <div style={{ width: 42, height: 42, borderRadius: "50%",
                      background: dark ? "rgba(255,255,255,0.08)" : faint,
                      border: dark ? `1px solid ${bar}55` : "none",
                      display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 }}>
                      {info.icon}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ margin: 0, fontSize: 13, fontWeight: 900, color: dark ? "#EAF0FF" : C.text }}>{info.title}</p>
                      <p style={{ marginTop: 3, fontSize: 13, color: dark ? "rgba(255,255,255,0.78)" : C.sub,
                        overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{item.memo || item.date || ""}</p>
                    </div>
                    <div style={{ textAlign: "right", flexShrink: 0 }}>
                      {xp > 0 && <p style={{ margin: 0, color: dark ? "#5FE2C5" : gold, fontWeight: 900, fontSize: 13 }}>{xpEmoji} +{xp}</p>}
                      {coin !== 0 && <p style={{ margin: "2px 0 0",
                        color: dark ? (coin > 0 ? "#5FE2C5" : "#FF8AA0") : (coin > 0 ? C.green : C.red),
