...
import { C, mixWhite, DUNGEON_DECOR_CARD, dungeonDecorRarity } from "../../data/tokens.js";
...
/* ════════════════════════════════════════════════════════════════════════
   ...
   같은 캐릭터 탭의 꾸미기 상점(DecorShopSheet)은 이미 숲 배경(GP)·통일된
   남색 카드+등급 테두리 글로우로 넘어갔는데 여기만 안 옮겨졌다. 꾸미기
   상점과 같은 재료(GP.appBg/headerBg, DUNGEON_DECOR_CARD)로 맞췄다 —
...
export default function ItemShopSheet({ open, onClose, dark, skin = "dungeon", coin = 0,
  rewards = [], hasPending, onRequest, themeMain = "#60A8FF", GP = {},
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 4000,
      ...
      <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 460, height: "88vh",
        ...
        <div style={{ position: "sticky", top: 0, zIndex: 5, padding: "16px 20px 14px",
          ...
          color: dark ? (GP.onDark || "#fff") : "#6B4A5C", flexShrink: 0, boxShadow: dark ? `0 4px 16px ${themeMain}22` : "none" }}>
          ...
          {dark && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: GP.chipBg, border: `1px solid ${GP.chipBorder}`, borderRadius: 14, padding: "5px 12px" }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 900, color: GP.chipText }}>{coin} {coinName}</span>
              </div>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: GP.chipBg, border: `1px solid ${GP.chipBorder}`, borderRadius: 14, padding: "5px 12px" }}>
                <span style={{ fontSize: 16 }}>🧾</span>
                <span style={{ fontSize: 14, fontWeight: 900, color: GP.chipText }}>총 구매 {approvedCount}개</span>
              </div>
            </div>
          )}
        </div>
            ...
            {rewards.slice().sort((a, b) => a.point - b.point).map(reward => {
              ...
              const statusPill = pending
                ...
                : canGet
                  ...
                  : { bg: "rgba(255,255,255,0.10)", text: "rgba(255,255,255,.6)", label: "모으는 중" };
              return (
                <div key={reward.id} style={cute
                  ...
                  : { position: "relative", borderRadius: 18, overflow: "hidden", background: cardBg,
                      border: `2px solid ${cardBorder}`, boxShadow: `${dr.glow}, 0 6px 16px rgba(8,16,40,0.42), inset 0 1px 0 rgba(255,255,255,0.06)`,
                      opacity: canGet || pending ? 1 : 0.9 }}>
                  <button onClick={() => setOpenId(isOpen ? null : reward.id)}
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 11, textAlign: "left" }}>
                      ...
                      <div>
                        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
                          <p style={{ fontSize: 17, fontWeight: 900, margin: 0, color: cute ? C.text : "#EAF0FF" }}>{reward.title}</p>
                        ...
                        <p style={{ fontSize: 13, fontWeight: 800, margin: 0, color: cute ? C.sub : "rgba(234,240,255,0.66)" }}>{reward.point} {coinEmoji} {coinName} 필요</p>
                      </div>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                      ...
                      {!cute && <span style={{ fontSize: 11, color: "rgba(234,240,255,0.55)",
                  ...
                  {isOpen && (() => (
                      <div style={{ padding: "0 14px 14px" }}>
                        <p style={{ fontSize: 13, fontWeight: 800,
                          color: cute ? (pending ? C.purple : canGet ? C.green : C.orange) : (pending ? "#D8CBFF" : canGet ? "#9FFFC4" : "rgba(234,240,255,0.55)"),
                          margin: "0 0 10px" }}>
                        ...
                        <button onClick={() => onRequest(reward)} disabled={!canGet || pending}
                          style={!cute
                            ? { width: "100%", padding: "11px 12px", borderRadius: 14, border: "none",
                                background: pending ? "rgba(167,139,250,.28)" : canGet ? "linear-gradient(135deg, #4EA3FF 0%, #78C7FF 100%)" : "rgba(255,255,255,.06)",
                                color: pending ? "#D8CBFF" : canGet ? "#fff" : "rgba(255,255,255,.4)", fontSize: 15, fontWeight: 900,
