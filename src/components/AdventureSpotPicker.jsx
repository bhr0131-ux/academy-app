/* ════════════════════════════════════════════════════════════════════════
   ...
   진행 상태는 '원판 테두리 색'으로만 표현 — 배지를 얹지 않아 깔끔하다.
     지나온 학원 : 원판 위 빨간 깃발 + 원판·글자 톤다운 (테두리는 남은 학원과 같다)
     현재 학원   : 골드 테두리 + 반짝임
     남은 학원   : 연갈색 테두리
    ...
    <div>
      ...
      <div key={ri} style={{ display: "flex", alignItems: "flex-start", justifyContent: "center", paddingTop: 12, width: "100%" }}>
        {row.map((it, i) => {
          ...
          /* [사용자 확정 2026-08-13] 지나온 학원의 초록 테두리는 뺐다 —
             원판 위 빨간 깃발과 원판·글자 톤다운만으로 이미 '다녀왔다'가 읽히는데,
             초록 링까지 붙으니 골드(현재)와 색이 경쟁해서 줄이 알록달록해졌다.
             지나온 칸도 남은 칸과 같은 연갈색 테두리를 쓴다. */
          const ring = it.current ? "#D9A441" : "rgba(155,114,74,0.38)";
          return (
            <Fragment key={it.id}>
