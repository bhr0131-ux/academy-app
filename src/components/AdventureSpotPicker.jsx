/* ════════════════════════════════════════════════════════════════════════
   ...
   진행 상태는 '원판 테두리 색'으로만 표현 — 배지를 얹지 않아 깔끔하다.
     지나온 학원 : 원판 위 빨간 깃발 + 원판·글자 톤다운 (테두리 없음)
     현재 학원   : 골드 테두리 + 반짝임  ← 테두리를 쓰는 건 여기 하나뿐이다
     남은 학원   : 테두리 없음 — 크림 원판에 학원 이모지만
    ...
    <div>
      ...
      <div key={ri} style={{ display: "flex", alignItems: "flex-start", justifyContent: "center", paddingTop: 12, width: "100%" }}>
        {row.map((it, i) => {
          ...
          /* [사용자 확정 2026-08-13] 테두리는 '현재 학원' 하나만 쓴다.
             먼저 지나온 학원의 초록 테두리를 뺐고(깃발·톤다운으로 충분), 이어서
             남은 학원의 연갈색 테두리도 뺐다 — 크림 원판에 학원 이모지만 얹은 모양.
             테두리가 골드 하나뿐이라 '지금 여기'가 한눈에 잡힌다.
             transparent 로 두는 이유: 두께(2.5px)는 남겨야 골드 칸과 크기가 같다
             (boxSizing:border-box 라 폭을 먹지 않고, 배경이 그 자리까지 칠해진다). */
          const ring = it.current ? "#D9A441" : "transparent";
          return (
            <Fragment key={it.id}>
