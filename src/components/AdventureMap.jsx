...
const MAP_LONG = {
  ...
  /* 동물 일곱 [중심x%, 바닥y%, 폭%] — 하루 두 마리만 나온다 (rollMapAnimals)
     [사용자 확정 2026-08-13] 폭은 20 으로 통일한다(예전엔 9~24로 제각각).
     단 두 종만 예외다 — 나비 14(20에서 30% 축소), 개구리 26(20에서 30% 확대).
     나비 원화는 거의 정사각형이라 폭 20이면 세로도 같이 커져 혼자 덩치가 커 보였고,
     개구리는 원화가 납작해(가로세로 1.59:1) 폭 20에서 유독 작아 보였다.
     높이는 원화 비율을 따라가므로 폭이 같아도 종마다 다르다 — 높이% = 폭% / (원화비율 × yr).
     자리(x·y)는 아직 예전 값이다. 짧은 지도만 사용자가 새로 찍어 줬다. */
  animals: { ev_parrot: [82, 30, 20], ev_monkey: [70, 54, 20], ev_toucan: [15, 74, 20],
             ev_boar: [85, 73, 20], ev_frog: [15, 86, 26],
             /* 나비·거북이는 예전 '손님' 자리(중심 기준)를 바닥 기준으로 환산해 옮겼다 */
             ev_butterfly: [28, 25.3, 14], ev_turtle: [15, 91, 20] },
...
const MAP_SHORT = {
  ...
  /* 동물 일곱 [중심x%, 바닥y%, 폭%] — 하루 두 마리만 나온다 (rollMapAnimals).
     [사용자 확정 2026-08-13] 자리 재배치 — 사용자가 격자 시트에 파란 점으로 찍어 준 자리 그대로다.
     예전엔 왼쪽 아래(x 12~15)에 큰부리새·개구리·거북이 셋이 몰려 박스가 서로 겹쳤다.
     큰부리새를 위쪽 잔디로(73 → 34.4), 나비를 가운데 폭포 옆으로(26.8 → 48.9),
     거북이를 오른쪽 아래 모래사장으로(12,90 → 68,87.9) 흩었다.
     원숭이는 오른쪽 가장자리로(72 → 90.3) 옮겨 가운데 학원 자리를 비웠다.
     앵무새·멧돼지는 찍힌 자리가 지금과 거의 같아 미세 보정만 했다.
     [사용자 확정 2026-08-13] 폭은 20 으로 통일한다(예전엔 10~24로 제각각).
     단 두 종만 예외다 — 나비 14(20에서 30% 축소), 개구리 26(20에서 30% 확대).
     높이는 원화 비율을 따라가므로 폭이 같아도 종마다 다르다 — 높이% = 폭% / (원화비율 × yr). */
  animals: { ev_parrot: [81.9, 31.2, 20], ev_monkey: [90.3, 56.8, 20], ev_toucan: [15.5, 34.4, 20],
             ev_boar: [87.8, 74.1, 20], ev_frog: [12.1, 79.2, 26],
             ev_butterfly: [31.6, 48.9, 14], ev_turtle: [68, 87.9, 20] },
