...
/* ── 무대 위 펫 가로 위치 ────────────────────────────────────────────────
   [사용자 확정 2026-09-26] 펫을 **캐릭터 몸 오른쪽에서 이만큼 띄운다**.
   예전엔 단계별 고정표(PET_DX = {1:62,2:76,3:86,4:98,5:102})였는데, 그 표는
   "진화할수록 원화 폭이 넓어진다"를 전제로 손으로 맞춘 값이었다. 성장 캐릭터
   원화를 새로 받으면서 그 전제가 깨졌다 — 새 원화는 3단계가 제일 넓고(162px)
   4단계가 제일 좁다(125px). 그래서 1단계에서는 펫이 부츠에 겹치고(-3px)
   4단계에서는 36px 이나 떨어져 있었다.
   이제 표를 안 쓰고 **그려진 캐릭터 폭을 재서** 반폭 + 이 값으로 자리를 잡는다.
   원화가 또 바뀌어도 다시 맞출 일이 없다.
   (아바타 모드는 정사각 캔버스라 폭이 거의 안 변해서 예전 값 62 를 그대로 쓴다) */
export const PET_GAP_FROM_CHAR = 16;   // 캐릭터 몸 오른쪽 끝 ~ 펫 사이 여백
export const PET_DX_AVATAR     = 62;   // 아바타(꾸미기) 모드 — 고정
export const PET_DX_FALLBACK   = 88;   // 아직 캐릭터를 못 쟀을 때(첫 한 프레임). 겹치는 것보다 먼 게 낫다
...
//   크기는 1024×2139 = 1:2.09 — 무대 칸이 세로로 아주 길어서(폰 폭에 상관없이 1:2.08~2.09)
//   2:3 그림을 쓰면 objectFit:cover가 양옆을 28% 잘라 먹었다. 칸 비율에 맞춰 잘림을 없앴다.
//   그림 자체가 이미 시안 톤이라 색 보정 없이 받은 그대로 넣는다.
...
export const DECOR_BGS = [
  ...
  /* [사용자 확정 2026-08-12] 이름은 '깊은 바다'.
     ...
     [원화 v6 로 교체 2026-08-12] 전체가 한 톤 밝아지고 물색이 옅어진 판이라 위 조절을 다시 잡았다.
       · 🐬 고래   : v6 위쪽에 고래가 크게 그려져 있다 → 겹치니 뺀다
       · 🫧 기포   : 좌우로 올라가는 기포 줄기가 v5보다 훨씬 촘촘하다 → 여섯에서 하나만 남긴다
       · 🐠 🐙 ✨ : 그림에 없는 것들이라 그대로 두고 생기를 맡긴다
  ...
  /* [원화 v7 로 교체 2026-08-12] 그림이 또 크게 바뀌었다 — 바위 협곡 사이로 난 넓은 모랫길,
     위쪽에 큰 고래 실루엣, 가운데 바다거북, 좌우에 산호·해초·조개, 멀리 작은 물고기 떼.
     v6에 촘촘하던 기포 줄기는 이 판에 아예 없다 → 🫧 를 다시 앞으로 올린다.
     🐬 는 v6부터 그림에 있어 계속 뺀 상태고, 🐙 ✨ 는 그림에 없어 그대로.

     [원화 v8 로 교체 2026-08-12] v7과 구도는 같고 생물이 늘었다 — 주황·은색 물고기가
...
export { HOLIDAYS, getHolidayName } from "./holidays.js";
