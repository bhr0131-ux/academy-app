...
/* ── 아이 화면 '종이색' [사용자 확정 2026-08-27 · 전체 색 점검] ────────────
   위쪽 수채화 무대 → 시트 → 탭 본문 → 바닥이 한 장의 종이처럼 이어져야 한다.
   원래 루트 배경 주석도 "시트와 같은 아이보리"였는데 값은 회청색(#F0F3F3)이라
   의도와 값이 어긋나 있었다 — 양피지 지도·크림 카드·수채화 원화가 그 위에서
   살짝 떠 보였다. 캐릭터 탭만 먼저 #FBF3ED 로 고쳤던 것을 전체로 넓힌다
   (그때 이유도 같았다: "원화 가장자리가 떠 보인다").
   sheet 는 시트·탭 본문·칩 바탕, bg 는 루트 바탕(아래로 살짝 가라앉는 그라데)이다. */
export const KID_PAPER = {
  sheet: "#FBF3ED",                                              // 시트·탭 본문
  chip:  "#F8F2E6",                                              // 안 고른 칩 (원래도 아이보리였다)
  bg:    "linear-gradient(180deg, #FBF3ED 0%, #F3E7DA 100%)",    // 루트 바탕
  onDeep:"#FFF8EF",                                              // 진한 바(날짜 바) 위의 글씨
};
