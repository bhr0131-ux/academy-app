/* ════════════════════════════════════════════════════════════════════════
   ...
   ────────────────────────────────────────────────────────────────────────
   기획 의도 (사용자 확정)
     "XP +50"만 뜨는 보상은 기억에 안 남는다. 지도 위를 걷다가 아주 짧게
     "오늘은 파란 나비를 발견했어!" 한 줄이 붙으면, 아이는 보상보다
     '오늘은 뭘 발견했을까?'를 기대하게 된다. 그 기대감이 이 앱의 핵심 재미다.
     발견은 미션과 무관하다 — '학원을 마치고 이동하며 발견 지점을 지나갔는가'
     (시간 기준)로만 정한다. 탐험 = 실제로 오늘 길을 다녀온 것 그 자체다.
...
/* ── 희귀도 (확률: 사용자 확정) ──────────────────────────────────────────
   ...
   [주의] 여기 weight는 '그 등급이 걸릴 확률'이지 '한 종류가 나올 확률'이 아니다.
   등급 안에서는 종류를 고르게 뽑으므로, 한 종류의 확률 = weight ÷ 그 등급 종류 수다.
   등급마다 종류 수가 다르니(35·12·9·3) weight만 보고 판단하면 안 된다.

   [사용자 확정 2026-08-03] 70/20/8/2 → 73/17/8/2 로 조정.
     예전 값은 일반 2.00% vs 희귀 1.67%로 거의 차이가 없었다. 일반에 35종이나
     몰려 있어서, 별 두 개짜리를 만나도 별 하나짜리와 체감이 같았다.
     종류 수(35·12·9·3)를 계산에 넣어, 한 종류당 확률이 등급마다 약 1.5배씩
     벌어지도록 다시 잡았다:
       일반 2.09% · 희귀 1.42% · 매우 희귀 0.89% · 전설 0.67%
     한 번 2배 사다리(80/14/5/1)로 올려 봤지만 59종을 다 모으는 데 1.7년이
     걸려 너무 길었다(사용자: "2년은 좀 길다"). 1.5배면 완성 기간이 예전과
     같은 약 1년으로 돌아오면서 등급 차이는 살아 있다.
     (등급 종류 수가 바뀌면 이 비율도 깨진다 — 종류를 늘리거든 weight를 다시 계산할 것) */
export const DISCOVERY_RARITY = {
  common:   { label: "일반",     star: "⭐",     color: "#8A7458", weight: 73 },
  uncommon: { label: "희귀",     star: "⭐⭐",   color: "#3B82F6", weight: 17 },
...
/* ── 무지개 (사용자 확정 2026-08-05) ────────────────────────────────────
   ...
   **드물게** 뜬다 — 하늘에 걸리는 것이라 자주 보이면 특별함이 없다.
   3.3%, 서른 날에 하루꼴이다(사용자 확정 2026-08-13). 동물 뽑기와 별개의 시드를 쓴다.
...
export function rollRainbow(childId, dateStr, eventId = null) {
  ...
  /* [사용자 확정 2026-08-13] 5% → 3.3% — '한 달에 한 번쯤'. 20일에 하루는 잦아
     특별함이 덜했다. 1000으로 나눠 33이면 30일에 하루꼴이다. */
  return hash32(`${childId}|${dateStr}|rainbow`) % 1000 < 33;
}
