...
/* ── 희귀도 (확률: 사용자 확정) ──────────────────────────────────────────
   ...
   [사용자 확정 2026-08-03] 70/20/8/2 → 80/14/5/1 로 조정.
     예전 값은 일반 2.00% vs 희귀 1.67%로 거의 차이가 없었다. 일반에 35종이나
     몰려 있어서, 별 두 개짜리를 만나도 별 하나짜리와 체감이 같았다.
     지금은 한 종류당 확률이 등급마다 약 2배씩 벌어진다:
       일반 2.29% · 희귀 1.17% · 매우 희귀 0.56% · 전설 0.33%
     (등급 종류 수가 바뀌면 이 비율도 깨진다 — 종류를 늘리거든 weight를 다시 계산할 것) */
export const DISCOVERY_RARITY = {
  common:   { label: "일반",     star: "⭐",     color: "#8A7458", weight: 80 },
  uncommon: { label: "희귀",     star: "⭐⭐",   color: "#3B82F6", weight: 14 },
  rare:     { label: "매우 희귀", star: "⭐⭐⭐", color: "#8B5CF6", weight: 5  },
  legend:   { label: "전설",     star: "🌈",     color: "#F59E0B", weight: 1  },
};
