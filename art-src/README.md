...
| bg-forest-v2-src.webp | 꾸미기 배경 '마법 숲' 원화 v2 (870×1807 = 1:2.077 — 무대 칸 1:2.09 와 거의 같아 좌우가 아주 조금 잘린다). v1(연민트 안개 숲, 하얀 여우)에서 색과 소품이 통째로 바뀌었다 — 연보라·연분홍 파스텔 숲, 가운데 빛나는 아치문이 난 큰 나무, 그 왼쪽에 하얀 사슴, 위쪽에 빛나는 나비, 좌우 앞쪽에 보라 수정·빛나는 버섯, 가운데로 난 밝은 흰 길. tint 연민트 rgba(150,220,205,0.10) → 연보라 rgba(200,180,245,0.10) — 그림이 라벤더 계열로 바뀌어 민트를 덮으면 색이 탁해진다. darkStage 는 v1과 같이 끔(위쪽이 밝은 연보라라 크림색 문구가 안 읽힌다) — 원화 없는 bakery(벚꽃 배경)에만 남아 있다. deco 목록은 손대지 않았다 — 장식 이모지를 화면에서 통째로 꺼 둔 상태(SHOW_BG_DECO_EMOJI)라 눈으로 맞춰 볼 수 없어서, 다시 켤 때 그림 보고 고른다. 받은 그대로 톤 보정 없음. art-src 는 q92(158KB), 배포본은 q80(71KB) | stage-bg-forest.webp |
