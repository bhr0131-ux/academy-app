...
| bg-deepsea-v8-src.webp | 꾸미기 배경 '깊은 바다' 원화 v8 (869×1810 = 1:2.083 — 무대 칸 1:2.09 와 사실상 같아 잘리는 데가 없다). v7과 구도는 같다(바위 협곡 사이 넓은 모랫길 · 위쪽 고래 실루엣 · 가운데 바다거북 · 좌우 산호·해초 · 수면 빛줄기) — 생물이 늘었다. 주황·은색 물고기가 앞쪽에 여럿 그려지고 왼쪽 아래에 해마가 들어왔으며, 대신 v7에 있던 조개가 빠졌다. deco 는 🫧 🐙 ✨ 🫧 🐚 — 물고기가 이제 그림에 또렷하게 여럿 있어 🐠 를 뺐고(v7 은 멀리 실루엣뿐이었다), 그림에서 빠진 조개를 🐚 로 대신 채웠다. 🐬 는 고래가 그려져 있어 계속 뺀 상태. tint 0.10 · darkStage 끔 — v6부터 같은 설정이다(위쪽이 흰 빛줄기라 크림색 문구가 안 읽힌다). 받은 그대로 톤 보정 없음. art-src 는 q92(206KB), 배포본은 q80(76KB) | stage-bg-deepsea.webp |
