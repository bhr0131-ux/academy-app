...
| bg-jungle-v1-src.webp | 꾸미기 배경 '정글 원정대' 원화 v1 (868×1812 = 1:2.088 — 무대 칸 1:2.09 와 사실상 같아 잘리는 데가 없다). 가운데로 난 모랫길과 징검다리 놓인 개울, 좌우로 큰 잎·덤불, 나무늘보·큰부리새·재규어 새끼·파란 나비. 그림이 장식으로 넣던 것을 대부분 그려서(🌴 🍃 🌿 🦜 🦋) deco 는 그림에 없는 셋만 남겼다: 🐒 🐍 🌺 (나무늘보는 있어도 원숭이는 없고, 꽃은 하나도 없다). tint 초록 0.30 → 연한 초록 0.10(원화가 이미 초록으로 꽉 차 같은 색을 덮으면 탁해진다). darkStage 는 끔 — 위쪽이 밝은 하늘·연둣빛이라 크림색 문구가 안 읽힌다. 원화 없는 bakery(민트 정원)에만 남겼다. 받은 그대로 톤 보정 없음. art-src 는 q92(221KB), 배포본은 q80(89KB) | stage-bg-jungle.webp |
