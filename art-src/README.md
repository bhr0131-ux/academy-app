...
**속옷 몸통 v2 (2026-08-14, 사용자 원화 `base-under-v2-src.webp` 723×1536 투명)** — 하의를 입으면
베이스 몸통을 이 그림으로 바꾼다(`AVATAR_BASE_BODY_UNDER_IMG`, AvatarViewer 의 `wearsBottom`).
**왜**: 베이스 몸통에 초록 반바지가 그려져 있어서 하의 장비는 그 초록이 1px도 안 새도록
'실제 다리보다 넉넉하게' 그려야 했다 → 바지가 벙벙했다. 덮을 초록이 없어지면 하의를
다리 굵기 그대로 그리면 된다(같은 날 하의 3종을 0.88배로 줄였다).
**v1(반팔 셔츠+양말)은 폐기** — 하루 만에 v2(민소매+맨발)로 교체했다.
v1 은 넣었다가 사용자 요청으로 한 번 되돌린 적이 있고, 그때 하의도 함께 원복했다.
**v2 의 두 가지 특징**
  · 민소매라 '하의만 입고 상의를 안 입은' 상태에서 팔·어깨가 드러난다.
  · 맨발이라 발끝이 기존(양말)보다 11~12px 아래로 내려간다(남 934→945 / 여 936→948).
    표시 크기에서 2px라 접지 그림자와의 어긋남이 안 보인다. 발끝을 억지로 맞추려면
    몸을 3.6% 줄여야 하는데 그러면 팔·다리가 기존 몸과 더 어긋난다(IoU 0.849 → 0.830).
**정렬** — README 위쪽 '색 변형 정렬법'과 같은 알파 IoU 최대화 탐색:
남아 0.361배 @(380,395) IoU 0.849 / 여아 0.369배 @(381,385) IoU 0.844.
남녀 같은 원화를 각자 몸통에 맞춰 따로 정렬했다.
