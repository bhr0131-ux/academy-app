...
| starter-tee-src.webp | **기본 반팔티 (사용자 원화 2026-08-15)** — 흰 반팔, 남녀 공용. **807**×1536 (베이스보다 넓은 캔버스) · 알파 상자 x70~733·y410~990 · 가로 중심 **401.5**. 몸보다 크게 그려져 있어 탑재 시 중심을 401.5→355 로 옮기고 줄인다. **배치 확정(남아 기준 2026-08-16)** — 배율 **0.58 균등** · 목선(알파 상자 위끝 y410)을 몸 **y638** 에 맞춘다. 화면에서 x160~550(폭 390) · y635~977 을 차지한다. 상의 슬롯 기본 지급 예정 | (미탑재) |
| starter-shorts-src.webp | **기본 반바지 (사용자 원화 2026-08-15)** — 초록 반바지, 남녀 공용. **807**×1536 · 알파 상자 x160~643·y654~963 · 가로 중심 **401.5**. **배치 확정(남아 기준 2026-08-16)** — 티셔츠와 달리 **비균등 배율 sx 0.6241 / sy 0.6448**, 허리(알파 상자 위끝 y654)를 몸 **y921** 에, 가로는 중심 401.5→355 에 **+44** 보정. 화면에서 x199~507(폭 308) · y919~1122 를 차지하고, 몸 밖으로 나가는 폭은 좌 15px·우 14px 이다. 원화가 몸보다 가로로 넓고 세로로 짧아 균등 배율로는 속옷이 비치거나 옆이 튀어나와, 폭·길이를 따로 잡았다. 하의 슬롯 기본 지급 예정 | (미탑재) |

**기본 옷 두 장을 겹칠 때** — 티셔츠 밑단 y977 이 반바지 허리 y919 보다 58px 아래라 겹친다.
**티셔츠가 위**로 간다(사용자 확정 2026-08-16) — 빼 입은 모양이 되고 반바지 허리밴드가 가려진다.
슬롯 기본 z 가 이미 하의 30 → 상의 35 라 별도 z 지정 없이 그대로 맞는다.
