...
| adventure-map-v13-src.webp | 탐험지도 긴 판 v13 (885×1777 — 학원 4곳 이상용). v12와 구도·모래길이 사실상 같고 그림만 다시 그렸다(선이 정리되고 물·풀 결이 부드러워졌다). 오른쪽 중턱에 있던 야자나무는 빠졌다. 길 중심선 90점을 새 그림에 얹어 확인 — 전부 길 위라 좌표는 손대지 않았다. 배포본이 163KB → 111KB 로 줄었다 | adventure-map.webp |
| adventure-map-short-v4-src.webp | 탐험지도 짧은 판 v4 (930×1692 = 1:1.819 — 학원 0~3곳용). v3(922×1706 = 1:1.849)보다 세로가 1.6% 짧아져 `ar`·`yr`·경로 가중치를 새 크기로 맞췄다. 그림은 긴 판 v13과 같은 결로 다시 그린 판이고 길 모양도 거의 같다(85점을 얹어 확인 — 전부 길 위). 오른쪽 중턱 야자나무는 여기서도 빠졌다. 배포본이 184KB → 112KB | adventure-map-short.webp |
...
| bg-deepsea-v11-src.webp | 꾸미기 배경 '깊은 바다' 원화 v11 (872×1804 = 1:2.069 — v10과 같은 크기, 무대 칸 1:2.09 와 거의 같아 좌우가 아주 조금 잘린다). 큰 틀(바위 협곡 사이 넓은 모랫길 · 위쪽 고래 실루엣 · 가운데 바다거북 · 왼쪽 아래 해마 · 좌우 산호·해초 · 수면 빛줄기)은 v10과 같지만 두 가지가 달라졌다 — ① 모랫길 끝에 **해저 아치문**이 새로 생겨 길이 그 안으로 이어진다(다른 배경들의 '문' 소품과 결이 맞는다), ② 물고기가 늘었다(파란 무리 두 떼 · 노란 물고기 두 마리). 물색도 v10의 거의 흰빛에서 다시 또렷한 하늘색으로 돌아왔다. 그만큼 결이 늘어 배포본이 41KB → 60KB. deco 🫧 🐙 ✨ 🫧 🐚 · tint 0.10 · darkStage 끔 — v10 그대로다(위쪽 수면이 흰빛이라 크림색 문구가 안 읽힌다). 받은 그대로 톤 보정 없음. art-src 는 q92(164KB), 배포본은 q80(60KB) | stage-bg-deepsea.webp |
...
| bg-forest-v3-src.webp | 꾸미기 배경 '마법 숲' 원화 v3 (872×1804 = 1:2.069 — 무대 칸 1:2.09 와 거의 같아 좌우가 아주 조금 잘린다). v2와 구도·소품은 같고 다시 그린 판이다 — 연보라·연분홍 파스텔 숲, 가운데 빛나는 아치문이 난 큰 나무, 그 왼쪽에 하얀 사슴, 위쪽에 빛나는 나비, 좌우 앞쪽에 보라 수정·빛나는 버섯, 가운데로 난 밝은 크림빛 길. v2보다 색이 진해지고 결이 늘었다 — 좌우 가장자리에 큰 나무 두 그루가 액자처럼 서고, 하늘의 노을(주황)이 또렷해졌으며 수정·버섯의 발광이 강해졌다. 그만큼 압축이 덜 먹어 배포본이 71KB → 121KB (배경 여섯 판 중 가장 무겁다). tint 연보라 rgba(200,180,245,0.10) · darkStage 끔 — v2 그대로다(위쪽이 밝은 분홍·연보라라 크림색 문구가 안 읽힌다). deco 목록도 손대지 않았다 — 장식 이모지를 화면에서 통째로 꺼 둔 상태(SHOW_BG_DECO_EMOJI)라 눈으로 맞춰 볼 수 없어서, 다시 켤 때 그림 보고 고른다. 받은 그대로 톤 보정 없음. art-src 는 q92(277KB), 배포본은 q80(121KB) | stage-bg-forest.webp |
