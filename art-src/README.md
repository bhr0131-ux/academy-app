...
## 옷·모자를 탑재했으면 반드시 — 자홍색 배경에 한 번 얹어 볼 것 (2026-09-25)

**입은 모습이 멀쩡해도 탑재본은 깨져 있을 수 있다.** 실제로 여섯 벌이 그랬다.
두 겹의 눈속임이 겹치기 때문이다.

  · **입은 모습**은 베이스가 뒤에서 채운다 — 옷에 뚫린 구멍으로 베이스의 흰 속옷이
    비쳐 블라우스처럼 보이고, 소매에 매달린 손 조각은 베이스의 진짜 손과 정확히
    겹쳐 안 보인다.
  · **상점 카드**는 배경이 흰색이라 흰 구멍·크림색 조각을 또 가려 준다.

그래서 둘 다 통과해도 문제가 남는다. 옷만 떼어 **자홍색(255,0,255)** 위에 얹으면
한눈에 드러난다 — 살색·크림·흰색 어느 것과도 안 겹치는 색이라서다.

```python
# 탑재본을 흰 배경(카드 모습)과 자홍 배경(구멍 검사) 나란히 보기
from PIL import Image
im = Image.open("public/assets/avatar/top/새옷.webp").convert("RGBA")
bb = im.getchannel("A").getbbox()
for bg in ((255,255,255,255), (255,0,255,255)):
    w = Image.new("RGBA", im.size, bg); w.alpha_composite(im)
    w.crop(bb).show()
```

같이 볼 숫자 두 개 —
  · **연결 조각 수**: 1이 정상. 2 이상이면 떨어져 나온 부스러기가 있다.
  · **안쪽 구멍**: 테두리에 안 닿는 투명 영역. 천에 뚫린 구멍이다
    (딸기 소풍은 42개 10,954px이었다).

**이 검사로 찾아낸 것** — 우주복 남·여(손·목·발 조각) · 딸기 소풍(구멍 42개+조각) ·
별빛 마법사(손·목·다리) · 해적 모자(**두 눈과 앞머리가 통째로**) · 딸기 밀짚모자(머리카락).
사파리 남·여, 하늘 나들이, 해적 옷, 기본티, 신발 5종, 가방 4종은 깨끗했다.

**원인은 하나다** — '입고 있는 그림에서 색으로 옷만 떼어내기'는 옷이 흰색·크림색이거나
살색과 가까우면 실패한다. 옷만 오려 낸 원화(알파에 옷만 담긴 것)를 받으면 이 문제가
아예 없다. **원화가 새 판으로 바뀌면 탑재본도 같이 다시 올릴 것** — 딸기 소풍·별빛
마법사는 원화가 이미 깨끗한 판으로 바뀌어 있었는데 탑재본만 옛날 것이었다.
...
확정값: 배율 **0.58** · 가로만 **1.06배**(원화가 베이스보다 좁다) · 깃 위끝 **y382**
→ 탑재 상자 (362,382)-(660,738). 365 는 노출이 더 적지만 깃이 목을 타고 올라가 어색하다.
**[2026-08-21] 화면에서 보니 옷이 조금 커서 한 단 줄였다** (0.62·1.12·380 → 0.58·1.06·382).
어깨 노출은 오히려 432 → 382 로 줄었다 — 폭이 줄면서 소매가 팔을 더 정확히 따라간다.
0.56·1.04 까지 줄이면 947 로 튀어 어깨 위에 맨살이 다시 보인다. 여기가 바닥이다.

**가랑이 틈은 옷을 내려도 안 막힌다 — 그 자리만 메운다 (2026-08-21)**
반바지 가랑이의 V 틈으로 베이스의 흰 속옷이 비쳤다. 옷을 아래로 내려 덮으려 했지만
V 자체가 같이 내려가서 안 막히고(2~14px 내려도 흰색 89→22, 대신 어깨 노출 382→1,720),
어깨만 크게 벌어진다. **위치는 그대로 두고 그 자리만 반바지 색으로 채운다.**
  범위 y600~710 · x494~536 (가랑이 V 만, 바깥 실루엣과 멀다)
  조건 '베이스가 불투명한데 옷 알파가 250 미만' → 반바지 색(214,160,95)으로 불투명 채움
  구멍뿐 아니라 **반투명 가장자리도 채워야 한다** — 안 그러면 흰 테두리가 남는다 (198px)
다리 사이 진짜 빈 공간은 베이스 알파가 0 이라 그대로 뚫려 있다.
**탑재 완료 (2026-08-20)** — 위 값 그대로 들어갔다. `safari-boy-outfit-src.webp`

**[2026-09-25 재탑재] 소매가 팔보다 굵어 팔이 뚱뚱해 보였다 — 가로 중심을 옮겨서 줄였다**
화면 크기(210px)에서 해적 옷과 나란히 놓으니 사파리만 소매가 헐렁했다. 실측하면
소매가 베이스 팔보다 y525~560 구간에서 좌 10~12px · 우 6~9px 밖으로 나가 있었다
(전 구간 합 **불룩 1,841px**). 예전에는 가로를 줄이면 어깨에 맨살이 드러나 못 줄였는데,
원인은 배율이 아니라 **가로 중심**이었다 — 원화가 왼쪽으로 1.5px 치우쳐 있어서
512 에 맞추면 한쪽이 먼저 모자란다. 중심을 **513** 으로 옮기니 같은 배율에서도
양쪽이 고르게 맞아, 가로를 더 줄일 여유가 생겼다.
| | 배율 | 가로배 | 깃 위끝 | 중심 | 불룩 | 맨살 |
|---|---|---|---|---|---|---|
| 예전 | 0.578 | 1.06 | 382 | 511.5 | 1,841 | 8 |
| **현행** | **0.578** | **1.047** | **386** | **513** | **740** | **38** |
→ 탑재 상자 (364,384)-(663,745). 불룩이 60% 줄고 맨살은 38px(팔 가장자리 실오라기)만 남는다.
**가랑이 메움은 다시 해야 한다** — 원화에서 새로 올리면 위의 V 틈이 같이 돌아온다.
같은 규칙으로 다시 채웠다: y600~725 · x486~546 에서 '베이스 불투명 + 옷 알파<250' 148px 을
반바지 색 (213,159,90) 으로 불투명 채움. 경로에 `?v=4` 를 붙여 기존 기기 캐시를 끊는다.

**[2026-09-25] 어깨 위 맨살 — 여아 옷 두 벌도 같은 증상이었다**
사용자가 사파리 여아에서 발견했다. 상의 전 벌을 재보니(베이스의 '살'만 골라, 깃 V 와
반팔 아래 팔은 빼고 y370~500 구간) 사파리 여아 180px · 해적 여아 115px 두 벌만 걸렸다.
남아 옷·딸기 소풍·별빛 마법사·하늘 나들이·우주복은 0 이다.
고치는 법은 남아 사파리와 같다 — **깃을 몇 px 올리고, 그만큼 짧아지는 소매·밑단은
세로 배율로 되돌린다.** 폭은 안 건드린다(건드리면 옷이 몸 밖으로 불룩해진다).
| 옷 | 예전 | 현행 | 어깨 노출 |
|---|---|---|---|
| 사파리 여아 | 0.829 · y372 | 0.829 · 세로 0.85 · y366 | 180 → 20 |
| 해적 여아 | 0.74 · y372 | 0.74 · 세로 0.755 · y368 | 115 → 6 |
※ 기본 반팔티 여아가 924px 로 잡히지만 **오검출**이다 — 반팔 아래 드러난 팔이라 정상.
...
| 아이템 | 배율 | 맞춘 기준 | 탑재 상자 |
|---|---|---|---|
| 사파리 옷 | ~~0.829 · y372~~ → **0.829 세로 0.85 · 깃 위끝 y366** | **[2026-09-25 수정]** 깃을 기본 반팔티와 같은 y372 에 맞췄더니 **어깨 위 양쪽으로 맨살이 비쳤다**(사용자 지적). 원화의 어깨선이 베이스보다 처져 있어서다 — 남아 사파리와 같은 증상이다. **깃을 6px 올려** 덮고, 그만큼 소매가 짧아지는 것은 세로만 0.85 로 2.5% 늘려 되돌렸다. 폭(0.829)은 안 건드렸다. 어깨 노출 180px → 20px | ~~(388,372)-(635,737)~~ → (387,366)-(637,743) |
...
| space-boy-src.webp | **우주복 원화 남아 (사용자 원화 2026-09-25)** — 543×897 캔버스에 **알파에 옷만** 담긴 그림(RGB 엔 손·목·발목 살까지 있지만 그 자리 알파는 0 — 해적 남아 원화와 같은 방식). 크림 우주복 + 남색 깃·벨트·소맷부리 + 금단추 + 별·행성 패치. 알파 상자 (10,10)-(533,887). 원화 몸이 베이스 v7 보다 팔다리가 길고 가늘어 **가로 0.62 · 세로 0.555 로 따로** 줬다(세로 11% 눌림) · 깃 위끝 y400 → 탑재 상자 (349,398)-(676,888) | avatar/top/space-suit-boy.webp |
| (딸기 소풍·별빛 마법사 재탑재 2026-09-25) | **원화가 언젠가 '옷만 오려 낸 깨끗한 그림'으로 바뀌어 있었는데 탑재본은 옛날 '색으로 떼어낸' 판 그대로였다.** 그래서 탑재본에는 손·목 조각과 **천에 뚫린 구멍**이 남아 있었다 (딸기 소풍: 안쪽 구멍 42개 10,954px — 깅엄의 흰 칸과 크림 블라우스가 통째로 날아가 있었다. 입은 모습에선 베이스의 흰 속옷이 블라우스 노릇을 해서 안 보였고, 상점 카드도 배경이 흰색이라 가려졌다). **패치가 아니라 깨끗한 원화에서 다시 올렸다.** 현재 자리를 지키려고 옛 탑재본과 색이 가장 잘 맞는 배율·오프셋을 찾아 썼다 (마법사 0.583/(263,-32) 색차 7.4 · 소풍 0.596/(259,-44) 색차 19.7, 덮임 97%). 새 상자 — 마법사 (353,356)-(667,755) · 소풍 (372,366)-(651,686). 경로에 `?v=2` 를 붙였다. **교훈: 원화가 새 판으로 바뀌면 탑재본도 같이 다시 올릴 것** | avatar/top/magic-outfit-girl.webp · picnic-outfit-girl.webp |
| (신발·가방 검사 2026-09-25) | **신발 5종·가방 4종 모두 깨끗했다** (조각·구멍 없음, 흰·자홍 배경 둘 다 확인). 다만 **아홉 개 다 상점에 없다** — 2026-08-20 사용자 확정으로 뺀 은퇴 아이템이고 그림만 남겨 둔 것이다(RETIRED_ITEM_INFO 참고). 하의 3종(크림·데님·카키 반바지)도 같다. 다시 넣게 되면 그대로 쓸 수 있다 | (미탑재 — 파일만 보관) |
| (모자 2종 청소 2026-09-25) | **해적 모자·딸기 밀짚모자** — 원화가 '쓰고 있는 머리 그림'이라 색으로 얼굴을 떼어냈는데 조각이 남아 있었다. 해적 모자는 **두 눈과 앞머리가 통째로** 붙어 있어 상점 카드가 '눈 달린 모자'였다. 쓴 모습에선 베이스 얼굴과 겹쳐 안 보여 여태 안 드러났다. 모자색(남색 b−r>8 / 주황 r−g>60 & r>200)으로 고갱이를 잡고 3px 부풀려 윤곽선을 되찾은 뒤, 가장 큰 덩어리만 남기고 챙 아래 갈색(r−b>25, r<195)을 털었다 — 5,966+920px. 밀짚모자는 같은 방식이 **챙 윤곽선까지 갉아먹어서**(윤곽 lum≈150, 머리카락 lum≈50) 밝기로 갈랐다: y≥158 에서 lum<120 & r−b>35 만 제거 (2,112px + 부스러기 39px). 사파리 머리띠는 10px뿐이라 그대로 뒀다. 경로에 `?v=2` | avatar/hat/pirate-hat-girl.webp · picnic-hat-girl.webp |
| hoodie-boy-src.webp | **민트 후드 원화 남아 (사용자 원화 2026-09-25 · 이름 '민트꽃 후드'에서 줄임 2026-09-26)** — 852×1846 캔버스에 **옷만 오려 낸 깨끗한 그림**(연결 조각 1개, 안쪽 구멍 0). 크림 후드 + 왼팔만 보라 소매 + 민트 후드 안감·꽃 패치 + 짙은 카고 반바지. 받은 파일에 캔버스 전체로 퍼진 **알파 1~59 짜리 찌꺼기 7,610px** 이 있어 알파<20 을 0 으로 눌러 보관했다(그 전 알파 상자 (57,37)-(829,1728) → 정리 후 (109,661)-(748,1412)). **후드 안쪽 목·가슴 살은 일부러 남겼다** — 후드 구멍이 베이스 목(폭 58~62)보다 훨씬 넓어 살을 지우면 뻥 뚫린다(우주복 깃과 반대 경우). 배율 가로 0.50 · 세로 0.45 · 후드 위끝 y396 → 탑재 상자 (350,394)-(674,737) · 소매부리 y635 · 반바지 밑단 y737 | avatar/top/hoodie-boy.webp |
| acorn-boy-src.webp | **도토리 탐험복 원화 남아 (사용자 원화 2026-09-26)** — 860×1829 캔버스에 **옷만 오려 낸 깨끗한 그림**(연결 조각 1개, 안쪽 구멍 0). 크림 셔츠 + 나뭇잎 망토 + 도토리 금브로치 + 가죽 벨트 + 갈색 카고 반바지. 민트 후드와 똑같이 캔버스 전체로 퍼진 **알파 1~19 짜리 찌꺼기 9,398px** 이 있어 눌러서 보관했다(정리 후 알파 상자 (79,527)-(781,1377)). **배율 0.42 균등** · 망토 위끝 y383 → 탑재 상자 (366,383)-(661,740). 배율은 **사파리 옷 남아에 맞춰** 잡았다 — 소매부리 폭 293px, 상자 x366~661 로 둘이 거의 같은 자리에 온다(사파리 (366,386)-(659,742)). 같은 몸에 입히는 같은 종류의 옷이라 자리가 어긋나면 갈아입을 때 튄다. **망토 깃 안쪽 목·가슴 살은 남겼다** — 깃 구멍(폭 39)이 베이스 목(폭 67)보다 좁아서 살을 지우면 깃 사이로 구멍이 보인다(민트 후드와 같은 이유, 우주복 깃과 반대). 자홍 검사 통과(조각 1·구멍 0) | avatar/top/acorn-outfit-boy.webp |
| growth-characters-760/adventure/{boy,girl}/stage-1~5.webp | **탐험 성장 캐릭터 5단계 남·여 (사용자 원화 2026-09-26 — 전면 교체)** — 받은 그림 10장이 전부 배경이 지워진 상태였고 **알파 1~19 짜리 헤일로**가 있었다(남아2 11,051px · 여아1 11,953px). **안 누르면 알파 상자가 그쪽까지 늘어나 캐릭터가 작게 들어간다** — 남아2 는 상자가 684 → 1042 로 벌어졌다. 눌러서 다시 잰 뒤 아바타 장비와 같은 규칙(연결 조각 60px 미만 제거 + 250px 이하 안쪽 구멍 메움)을 돌렸다. 겨드랑이·다리 사이처럼 **큰 빈 곳은 진짜 여백이라 그대로 둔다** — 옷과 달리 캐릭터는 구멍이 있는 게 정상이다. 보관 760 높이 · 배포 500 높이(알파 상자에 맞춰 자르고 세로만 맞춘다, 가로는 비율). **진화할수록 물건이 하나씩 는다** — 1 기본(사파리 헬멧·노란 조끼·초록 반바지·부츠) → 2 +고글 → 3 +허리 파우치 → 4 +빨간 스카프 → 5 +별 뱃지. 포즈도 단계마다 다르다(차렷 → 경례 → 가리키기 → 허리에 손 → 주먹). 예전 판(지도·돋보기·망토·지팡이가 늘어나던 세트)은 규칙 6 에 따라 같은 파일명으로 덮어썼다. CHAR_IMG 의 `?v=` 를 5 → 6 으로 올렸다 | assets/growth-characters/adventure/{boy,girl}/stage-1~5.webp |
| (여아 전수 점검 2026-09-25) | 사용자 요청으로 **여아가 볼 수 있는 옷·모자 10종을 전부** 훑었다(입은 모습 + 상점 카드 + 자홍 배경 + 수치). 눈에 띄는 흠은 없었지만 수치로 **부스러기와 1픽셀급 구멍**이 남아 있어, 아바타 장비 전 파일에 같은 규칙을 한 번에 돌렸다 — **연결 조각 60px 미만 제거 + 테두리에 안 닿는 250px 이하 구멍을 이웃 색으로 메움**. 17개 파일이 손질됐다(해적 모자 부스러기 40·구멍 191 / 해적 옷 여아 구멍 73 / 별빛 마법사 부스러기 41 / 사파리 여아 부스러기 5·구멍 48 / 딸기 소풍 부스러기 7·구멍 22 …). 가장자리가 반투명이라 첫 판에 안 채워진 3개는 이웃 기준을 알파 60 으로 낮춰 마무리했다. **결과: 상점에 있는 17개 파일 전부 조각 1개·부스러기 0·구멍 0.** 손질로 그림이 바뀐 8종은 썸네일을 다시 뽑고 `?v=` 를 올렸다. 은퇴 파일(비행모·꽃헬멧·탐험헬멧·크림부츠·탐험끈·데님/카키 반바지) 8개도 같은 규칙에 함께 손질됐다 — 상점엔 없지만 배포본에는 들어가므로 그대로 뒀다 | (전 파일) |
| space-girl-src.webp | **[2026-09-25 탑재본 청소]** 색으로 옷을 떼어낸 흔적(손·목·발 살 조각, 깃 안쪽 목)이 층에 남아 있었다. 입은 모습에선 베이스의 같은 자리와 겹쳐 안 보였지만 **상점 카드에선 옷에 손이 매달린 것처럼 보였다.** 소매부리 y583 아래 바깥 덩어리·바지밑단 y868 아래·깃 안쪽 y346~374 의 살과 머리카락을 지우고(3,812px + 부스러기 9조각 22px) 썸네일을 다시 뽑았다. 남아 것도 깃 안쪽 목(y400~428, 1,608px)을 같은 이유로 지웠다. 깃 안은 베이스의 제 목이 채우므로 구멍이 생기지 않는다(실측 확인). **우주복 원화 여아 (사용자 원화 2026-08-19)** — 베이스 v7 여아가 **입고 있는 전신 그림**(897×1753, 배경 검정). 크림색 우주복 + 연보라 어깨·벨트·무릎보호대 + 금색 지퍼·별 패치. 팔다리까지 덮는 한 벌 | avatar/top/space-suit-girl.webp |
...
| safari-boy-outfit-src.webp | **사파리 옷 원화 남아 (사용자 원화 2026-08-20)** — 864×1820 캔버스에 **옷만 오려 낸 그림**(알파 상자 (177,684)-(662,1299)). 크림 셔츠 + 탄색 카고 조끼 + 카고 반바지. 배율 0.58 · 가로만 1.06배 · 깃 위끝 y382 → 탑재 상자 (362,382)-(660,738). 그림이 성별로 갈리는 첫 아이템(top_vest 의 img=남아 / imgGirl=여아) | avatar/top/safari-outfit-boy.webp |
...
| expedition/ride/cloud-src.webp | 탑승 원화 15 구름 남아 — 초원·사막·보물 변형 (하늘 lift 7). **2026-08-08 재제작 137×153 → 280×310** (표시 77×86px의 3.5배 — 실측으로 정한 값). 여아 판도 같은 높이 310으로 다시 뽑아 둘이 같은 해상도다. **2026-08-26 알파 복구** — 재제작본이 투명 배경을 잃고 검은 사각형으로 나오고 있었다(32종 중 이 한 장만). 137×153 판에 남아 있던 알파를 280으로 올려 이식하고, 검정에 곱해진 색을 알파로 나눠 되살렸다(un-matte). 그림·구도는 그대로 | expedition/ride/cloud.webp |
...
**펫 진화 그림 (사용자 원화 2026-08-24)** — PET_STAGES 5단계 전부 그림을 받아
이모지 대신 그림으로 그린다(gameData.jsx의 PET_STAGE_IMG). 알파 있는 PNG를 받아
bbox로 여백만 잘라내고 배포본은 높이 480px로 축소했다(전부 정사각에 가까운 원화라
가로는 비율대로 따라간다). PET_STAGE_IMG에 그 단계 키가 없으면 화면이 자동으로
이모지로 되돌아가는 폴백 구조라, 앞으로 그림이 또 바뀌어도 코드는 안 건드린다.
등장하는 자리 셋 다 이 규칙을 따른다 — 무대 위 알/펫(HeroStage), 펫 소개 시트의
큰 그림·진화 단계 줄(PetSheet), 보물상자를 열어 부화·진화할 때 뜨는 이벤트 팝업
(App.jsx showGameEvent — 레벨업 진화 팝업과 같은 '이전→진화 그림' 틀을 그대로 썼다).
2~4단계는 이름·설명만 보고 어울리는 색으로 짝지었다 — 청룡(이름 그대로 파랑,
팔짱 낀 늠름한 자세) · 전설(수정 장식 얹은 보라, 반짝이는 눈).

**[사용자 확정 2026-08-24] 1·2단계 교체** — 처음 짝지었던 아기 드래곤(양팔 들고
웃는 포즈)·개구쟁이 드래곤(같은 포즈, 다른 색)은 사용자가 새 원화 두 장으로
바꿔 줬다. 규칙 6에 따라 구버전 원본(stage1/2-dragon-src.webp)은 지우고
새 원화를 v2로 넣었다 — 앉아서 웃는 차분한 쪽을 1단계(아기)에, 폴짝 뛰며
입 벌려 웃는 역동적인 쪽을 2단계(개구쟁이)에 맞췄다.

| 원화 | 용도 | 탑재 에셋 |
|---|---|---|
| pet/egg-src.webp | 0단계 '신비한 알' (1254×1254 → bbox 크롭 1168×1223) | pet/egg.webp |
| pet/stage1-dragon-v2-src.webp | 1단계 '아기 드래곤' v2 (1254×1254 → bbox 크롭 1187×1201, 앉아서 차분히 웃는 포즈) | pet/stage1.webp |
| pet/stage2-dragon-v2-src.webp | 2단계 '개구쟁이 드래곤' v2 (1254×1254 → bbox 크롭 1240×1240, 폴짝 뛰며 입 벌려 웃는 포즈) | pet/stage2.webp |
| pet/stage3-dragon-src.webp | 3단계 '늠름한 청룡' (1230×1278 → bbox 크롭 1216×1270) | pet/stage3.webp |
...
| 원화 | 용도 | 탑재 에셋 |
|---|---|---|
| petskin/fox-src.webp | 펫 스킨 '불꽃 여우' (1254×1254 → bbox 크롭 1240×1221) | petskin/fox.webp |
