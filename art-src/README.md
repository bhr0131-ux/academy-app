...
| bg-deepsea-v6-src.webp | 꾸미기 배경 '깊은 바다' 원화 v6 (887×1774 = 1:2.00). v5보다 전체가 한 톤 밝고 맑아진 판 — 물색이 옅어지고 위쪽 빛줄기가 흰색에 가깝게 세졌다. 위쪽에 고래가 크게 들어오고 모랫길도 넓어졌다. 이 밝기 때문에 코드도 같이 바꿨다: darkStage 를 끄고(위가 흰 빛줄기라 크림색 문구가 안 읽힌다), 🐬 는 그림에 있어서 빼고, 🫧 는 여섯에서 하나만 남기고, tint 는 0.16 → 0.10. 무대 칸(1:2.09)보다 납작해 objectFit:cover 로 좌우가 각 2% 정도 잘리는데, 잘리는 자리가 물색·모래라 티가 안 난다. 받은 그대로 톤 보정 없음. art-src 는 q92(211KB), 배포본은 q80(81KB) | stage-bg-deepsea.webp |
...
| bg-space-v2-src.webp | 꾸미기 배경 '우주 탐사' 원화 v2 (868×1810 = 1:2.085 — 무대 칸 1:2.09 와 사실상 같아 잘리는 데가 없다). v1(달 기지)에서 '우주에서 본 지구'로 구도가 통째로 바뀐 판 — 아래쪽 지구의 둥근 지평선이 그대로 아이가 서는 바닥이 된다. 하늘엔 보름달·토성·작은 위성 둘·은하수, 가운데를 로켓이 가로지른다. 그림이 장식 이모지로 넣던 것을 거의 다 이미 그려서(🚀 🪐 🌎 🌙 ⭐ 🌌) deco 는 그림에 없는 셋만 남겼다: ☄️ 🛸 🌠. tint 0.12 · darkStage 유지(밤하늘이라 응원문구는 크림색). **배포본만 q74** — 이 그림은 화면 전체가 별·수채 입자라 q80 이면 221KB 로 다른 배경(78~97KB)의 2배가 넘는다. 300% 확대로 q68·q74·q80 을 나란히 봤을 때 차이가 없어 q74(170KB)로 낮췄다. 받은 그대로 톤 보정 없음. art-src 는 q92(504KB) | stage-bg-space.webp |
