...
| expedition/char/{common,girl}-sit-src.webp | 앉기 포즈 남녀 원화 (2026-08-09, 남아 278×459 / 여아 493×716 — 받은 순서가 여아·남아라 처음에 반대로 넣었다가 바로잡았다). **쓰는 곳은 '쉬는 날 무대'다** — 탈것용이 아니다. 탈것 원화 32종에 이미 아이가 함께 그려져 있어(RIDE_READY·RIDE_GIRL_READY 각 32) 앱은 탈 때 탈것 한 장만 그리고, 어느 챕터도 pose:"ride"를 안 쓴다. 오늘 미션이 0개인 날 HeroStage가 진화 캐릭터 대신 이 그림을 서 있을 때 높이의 0.78배로 그린다(ADV_SIT_IMG·ADV_SIT_RATIO, 탐험 스킨·성장 캐릭터 모드 한정). 탑재는 **줄인 뒤에 halo 재색칠** — 리샘플이 가장자리에 밝고 반투명한 픽셀을 새로 만들어서, 원본에서 먼저 고쳐도 줄이면 다시 생긴다(남아 0 → 281개 실측). 여아는 676px 높이(표시 최대 193px의 3.5배)로 줄이고, 남아는 원본이 459px뿐이라 그대로 쓴다 | expedition/char/{common,girl}-sit.webp |
