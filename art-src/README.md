...
땋은 머리까지 붙어버려 실패한다).
**신발 탑재 공식 — 발목 앵커 방식 (사용자 확정)**
신발마다 좌표를 따로 잡지 말 것. **베이스 발 중심 x와 공통 바닥선 y를 앵커로 고정**하고
모든 신발을 거기에 맞춘다. 새 신발이 와도 같은 값으로 한 번 돌리면 끝난다.
  · 좌 발 중심 x=444 · 우 발 중심 x=568 (남녀 베이스 발 중심의 평균)
  · 밑창 y=936 (맨발 바닥 남 934·여 936과 같은 줄)
  · 배율 0.96 (밑창 두꺼운 디자인이라 그대로 두면 키가 커 보임)
**좌·우를 따로 옮길 것** — 원화는 두 짝이 한 장에 그려져 있는데 좌우가 대칭이 아니다.
연결요소로 두 짝을 분리해 각각 앵커에 맞춰야 한다(실측: 오른쪽 짝이 12px 바깥으로 치우쳐
있었다). 옛 방식(에셋 bbox 안에 비율 유지로 넣고 바닥만 맞추기)은 이 치우침을 못 잡는다.
**리사이즈 뒤에는 halo 보정을 다시 돌릴 것** — 축소하면 투명 픽셀의 흰 RGB가 번져
흰 테두리가 되살아난다(실측: 신발 3장에서 1,670px 재발생).
