...
**속옷 몸통 (2026-08-14, 사용자 원화 `base-under-v1-src.webp` 860×1828)** — 하의를 입으면
...
'실제 다리보다 넉넉하게' 그려야 했다 → 바지가 벙벙해 보였다. 덮을 초록이 없어지면
앞으로 하의는 다리 굵기 그대로 그리면 된다.
받은 파일은 **알파 없는 RGB(검은 배경)** 라 테두리에서 번지며 검정을 지워 투명으로 만들었다.
정렬은 README 위쪽 '색 변형 정렬법'과 같은 **알파 IoU 최대화 탐색** — 남아 0.293배 @(380,404)
IoU 0.898 / 여아 0.300배 @(380,395) IoU 0.894. 남녀 같은 원화를 각자 몸통에 맞춰 따로 정렬했다.
차이는 허벅지(옛 반바지가 더 넓음)와 어깨 몇 px 뿐이라 머리·팔·양말·신발이 그대로 맞는다.
