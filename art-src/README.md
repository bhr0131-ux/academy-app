...
| expedition/ride/{키}-girl-src.webp | 탐험 씬 탈것 여아 원화 23종 (2026-08-08). 탈것 그림에는 타고 있는 아이도 같이 그려져 있어 성별이 갈린다. 대개 남아 판과 크기가 거의 같아 그대로 탑재하지만, 크게 온 것은 **남아와 같은 높이로 줄인다**(거북이 487→302 · 구름 497→153 등) — 그림은 높이로만 크기가 정해지므로 높이를 맞춰야 두 성별이 같은 해상도로 보인다. halo 재색칠은 전부 적용. 남은 9종은 여아 원화가 없어 공용 그림을 쓴다 — 오면 RIDE_GIRL_READY에 키만 추가 | expedition/ride/{키}-girl.webp |
