...
| expedition/ride/{키}-girl-src.webp | 탐험 씬 탈것 여아 원화 27종 (2026-08-08). 탈것 그림에는 타고 있는 아이도 같이 그려져 있어 성별이 갈린다. 대개 남아 판과 크기가 거의 같아 그대로 탑재하지만, 크게 온 것은 **남아와 같은 높이로 줄인다**(거북이 487→302 · 구름 497→153 · 모래보드 475→238 등) — 그림은 높이로만 크기가 정해지므로 높이를 맞춰야 두 성별이 같은 해상도로 보인다. halo 재색칠은 전부 적용. 남은 5종(고래·잠수함·얼음미끄럼·순록썰매·열기구)은 공용 그림 — 오면 RIDE_GIRL_READY에 키만 추가 | expedition/ride/{키}-girl.webp |
...
| camp/tent-v1-src.webp | 캠프 텐트 (천막 면·꼭대기 깃발이 비어 있어 글자를 앱이 얹는다. 투명, 여백 잘라 1198×1130 → 화면용 1120×1056으로 축소 탑재. 글자 자리는 원화 픽셀에서 물 채우기로 실측 — 천막 면 x19.9~79.5%·y41.0~85.0%, 깃발 x52.3~75.5%·y3.7~18.4%) | camp/tent.webp |
