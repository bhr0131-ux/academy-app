...
| starter-tee-src.webp | **기본 반팔티 (사용자 원화 2026-08-15)** — 흰 반팔. **807**×1536 (베이스보다 넓은 캔버스) · 알파 상자 x70~733·y410~990 · 가로 중심 **401.5**. 몸보다 크게 그려져 있어 탑재 시 중심을 401.5→355 로 옮기고 줄인다. 배치는 아래 표 참고 — **남녀 값이 다르다**. 상의 슬롯 기본 지급 예정 | (미탑재) |
| starter-shorts-src.webp | **기본 반바지 (사용자 원화 2026-08-15)** — 초록 반바지. **807**×1536 · 알파 상자 x160~644·y654~964 · 가로 중심 **401.5**. 원화가 몸보다 가로로 넓고 세로로 짧아 **균등 배율로는 못 맞춘다** — 균등하게 맞추면 속옷이 비치거나 옆이 튀어나와서 폭(sx)과 길이(sy)를 따로 잡았다. 배치는 아래 표 참고 — **남녀 값이 다르다**. 하의 슬롯 기본 지급 예정 | (미탑재) |
...
#### 지금 탑재된 값은 위 표가 아니다 — v6 몸에 맞춘 임시값 (2026-08-16)

위 표는 **베이스 v7 기준**이고, v7 은 아직 안 올라갔다(머리·목 분리 작업 대기 중).
그런데 베이스 몸통이 '민소매+속옷+맨발' 한 장이라 아무것도 안 입히면 속옷 차림으로 보여서,
**지금 배포 중인 v6 몸에 따로 맞춰** 먼저 올렸다. 1024 캔버스 기준 값:

| | 배율 | 세로 기준선 | 탑재 상자 |
|---|---|---|---|
| 반팔티 | 0.4724 균등 | 목선 y346 | x355~669 · y345~621 |
| 반바지 | sx 0.4132 / sy 0.4839 | 허리 y580 | x412~612 · y581~730 |

v6 몸 실측: 목 y340 · 가랑이 ~y630 · 다리 y640~900 · 발 y900~940 (남녀 공용 한 장).
원화가 v7 치비 비율이라 v6 의 슬림·긴 상체에는 균등 배율로 안 맞아 반바지는 폭·길이를 따로 잡았다.
티셔츠 밑단 y621 이 반바지 허리 y581 을 40px 덮어 속옷이 안 보인다.

**v7 로 갈아탈 때** 위 v7 표 값으로 두 그림만 1024 캔버스에 다시 렌더해 같은 경로에 덮어쓰면 된다
(`public/assets/avatar/top/starter-tee.webp`, `bottom/starter-shorts.webp`). 카탈로그는 손댈 필요 없다.
