...
| bg-jungle-v2-src.webp | 꾸미기 배경 '정글 원정대' 원화 v2 (868×1812 = 1:2.088 — 무대 칸 1:2.09 와 사실상 같아 잘리는 데가 없다). v1과 구도가 통째로 다르다 — 아래쪽 개울을 건너는 큰 징검다리가 화면 절반을 차지하고 아이가 그 돌 위에 선다. 길은 안쪽으로 이어지고 그 끝에 코끼리, 왼쪽 나무에 나무늘보, 오른쪽 가지에 큰부리새, 멀리 산. v1보다 전체가 연하고 뿌옇다(안개 낀 아침 톤). 그림이 장식으로 넣던 것을 대부분 그려서(🌴 🍃 🌿 🦜) deco 는 그림에 없는 셋만 둔다: 🐒 🐍 🌺 (나무늘보·코끼리는 있어도 원숭이는 없고, 꽃은 하나도 없다). tint 연한 초록 0.10 · darkStage 끔 — v1 설정 그대로다(위쪽이 밝은 연둣빛이라 크림색 문구가 안 읽힌다). 받은 그대로 톤 보정 없음. art-src 는 q92(263KB), 배포본은 q80(108KB) | stage-bg-jungle.webp |
