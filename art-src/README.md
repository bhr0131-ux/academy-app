...
| bg-deepsea-v9-src.webp | 꾸미기 배경 '깊은 바다' 원화 v9 (869×1810 = 1:2.083 — 무대 칸 1:2.09 와 사실상 같아 잘리는 데가 없다). v8과 구도·소품은 그대로다(바위 협곡 사이 넓은 모랫길 · 위쪽 고래 실루엣 · 가운데 바다거북 · 왼쪽 아래 해마 · 좌우 산호·해초 · 수면 빛줄기) — 오른쪽에 있던 물고기 두 마리가 빠져 앞쪽이 트이고 모랫길이 넓어졌다. deco 🫧 🐙 ✨ 🫧 🐚 — v8 그대로다(물고기·거북·고래·해마는 그림에 있고, 기포·문어·조개는 없다). tint 0.10 · darkStage 끔 — v6부터 같은 설정이다(위쪽이 흰 빛줄기라 크림색 문구가 안 읽힌다). 받은 그대로 톤 보정 없음. art-src 는 q92(198KB), 배포본은 q80(75KB) | stage-bg-deepsea.webp |
...
| bg-forest-v1-src.webp | 꾸미기 배경 '마법 숲' 원화 v1 (864×1821 = 1:2.107 — 무대 칸 1:2.09 와 거의 같아 위아래가 아주 조금 잘린다). 안개 낀 연민트빛 숲, 가운데 큰 나무, 왼쪽에 하얀 여우, 빛나는 버섯, 반짝임, 가운데로 난 길. 이로써 꾸미기 배경 여섯 판이 모두 원화가 됐다. tint 초록 0.28 → 연민트 0.10(그림이 이미 연민트로 옅게 깔려 있어 진한 초록을 덮으면 안개가 탁해진다). darkStage 는 끔 — 위쪽이 거의 흰빛이라 크림색 문구가 안 읽힌다. 원화 없는 bakery(벚꽃 배경)에만 남겼다. deco 목록은 손대지 않았다 — 장식 이모지를 화면에서 통째로 꺼 둔 상태(SHOW_BG_DECO_EMOJI)라 눈으로 맞춰 볼 수 없어서, 다시 켤 때 그림 보고 고른다. 받은 그대로 톤 보정 없음. art-src 는 q92(166KB), 배포본은 q80(57KB) | stage-bg-forest.webp |
