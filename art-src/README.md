...
**속옷 몸통 (2026-08-14, 사용자 원화 `base-under-v1-src.webp` 860×1828) — 지금은 안 쓴다(보류)**
하의를 입으면 베이스 몸통을 속옷 차림으로 바꿔서 '하의가 초록 반바지를 덮을 필요'를 없애려던
시도였다. 넣었다가 **사용자 요청으로 같은 날 원상 복구**했다(몸은 초록 반바지 그대로).
되돌릴 때 하의 3종도 원래 크기로 함께 복구했다 — 속옷 몸통을 전제로 0.88배 줄여 놨던 터라,
몸만 되돌리면 초록이 800~3,100px 비친다(실측).
원화는 여기 남겨 둔다. 다시 쓰려면 (1) 이 파일을 남녀 몸통에 각각 알파 IoU 최대화로 정렬해
`public/assets/avatar/base/body-under{,-girl}.webp` 로 넣고 (2) AvatarViewer 가 하의 착용 여부를
보고 몸통 장을 고르게 하고 (3) 하의를 다리 굵기에 맞춰 다시 줄이면 된다.
당시 정렬값 — 남아 0.293배 @(380,404) IoU 0.898 / 여아 0.300배 @(380,395) IoU 0.894.
받은 파일이 알파 없는 RGB(검은 배경)라 테두리에서 번지며 검정을 지워야 한다.
